...
### Fixed
- **CF-236 SpotBugs closure gate**: Maven `verify` and standalone SpotBugs scans now use a checked-in known-debt filter and fail on new unbaselined findings, making SpotBugs blocking while preserving the remaining historical debt for follow-up slices.
...
- **SpotBugs remediation**: Cleared targeted baseline entries for realtime bar equality, serializable bar builder factories, concurrent trade-builder ownership, walk-forward calibration map iteration, charting/cache path guards, forced garbage collection in backtest batching and benchmark harness flows, `DoubleNumFactory` singleton serialization, descriptor-only chain links, `DoubleNum`/`NaN` comparisons, performance-report ordering, lower-camel criteria factories, constructor-safety validation paths, backtest execution, bar-series manager/sizing context snapshots, support/resistance, ATR true-range ownership, average, MACD/PPO, edge/momentum helper, scoring/compression, oscillator, VWAP-derived, Elliott source-indicator, facade, and walk-forward runner ownership, and Elliott scenario indicators, triggered/Elliott/time-rule and XOR/chain/one-shot/not-rule constructor validation, composite/staged rule accessor ownership, strategy/chain-link accessor ownership, backtest strategy snapshot ownership, indicator bar-series accessor ownership, Elliott suite analysis-result ownership, `BaseBar` time resolution nullness, obsolete Elliott macro demo private bridges, Renko, Alligator, fractal, swing-marker, and Marubozu constructor validation, base bar-series/cache constructor validation, and trade/trading-record constructor validation.
...
- **Elliott anchor calibration has a dedicated runner path**: The BTC anchor-calibration flow now runs through the explicit `ElliottWaveAnchorCalibrationHarness` examples job, keeping the multi-hour anchor analysis out of hosted PR verification and report-producing analysis-demo runs.
...
- **Contributor verify and example runs now line up with the repo’s real commands**: You can now run the Maven `verify` gate to mirror CI’s blocking SpotBugs plus advisory JaCoCo feedback for `ta4j-core` and `ta4j-examples`, then launch examples straight from repo root with `./mvnw -pl ta4j-examples exec:java` for `Quickstart` or `-Dexec.mainClass=...` for a specific demo (`CF-119`).
...
- **Tagged tests use explicit execution lanes**: `benchmark`, `integration`, and `analysis-demo` JUnit tags now have dedicated GitHub Actions workflows or documented local commands, so long-running suites stay out of push CI without becoming hard to exercise.
...
- **PR validation stays fast while the Elliott anchor job remains runnable**: GitHub build CI now keeps `integration` and `analysis-demo` excluded by default instead of running every tagged test on each PR, and contributors can still run the BTC anchor-calibration job through `ElliottWaveAnchorCalibrationHarness` when they need full validation.

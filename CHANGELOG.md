...
- **Parameter research pipeline hardened (`CF-455`)**: `ParameterResearch.Builder` now binds the candidate factory into a staged `CandidateStage<T>` that carries the factory's type, so maximize/minimize/run are only reachable with a correctly typed objective and a divergent candidate/objective pair is a configuration-time error instead of a bridge `ClassCastException` inside the run, and a second `candidate(...)` binding on the same builder is rejected so a retained stage can never rebind a differently typed workflow; normalizer and validator callbacks are re-invoked for every proposal event and raw re-proposals count against the run-wide proposal cap, so genetic elite slots and swarm re-projections keep stateful callback semantics while their callback work stays bounded; decimal canonicalization now scans the exact declared-position window (floored bounds, no clamping past the last declared index) so normalizer-mapped values beyond the declared range keep their raw identity instead of colliding with the last declared value; particle-swarm moves stop once the sweep cursor has no unseen cell left; and a canceled run finalizes the observed batch only when that batch actually processed proposals.

...
- **Cross-platform non-interactive chart test JVMs**: `ta4j-examples` Surefire now sets `ta4j.chart.disableDisplay=true` on all platforms and adds `apple.awt.UIElement=true` on macOS so charting-related tests avoid focus-stealing UI windows.

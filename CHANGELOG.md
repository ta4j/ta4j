...
- **Criteria equity curves compute in one pass and reuse cached state**: `CashFlow`, `CumulativePnL`, and `InvestedInterval` now build their curves in a single forward pass and reuse previously computed state, so repeated criterion evaluation no longer redoes the same work. Measured over eight criteria on a ~1,100-bar daily series with seven closed positions, individual evaluation runs about 1.4x faster (DoubleNum) and 1.15x faster (DecimalNum); the win grows with trade count because curve cost no longer scales with positions times window length. Windowed `CashFlow(series, record, startIndex, finalIndex, ...)` curves now match the full curve over the window, carrying the realized return of positions closed before `startIndex` instead of restarting at one.
- **Analysis curves are isolated from later bar edits**: `CashFlow`, `CumulativePnL`, `InvestedInterval`, and shared `EquityCurveCache` curves compute from a private copy of the series' bar data, so in-place edits of retained bar references can neither alter nor mix already-produced curves. Editing, appending, or removing bars, recording trades, or swapping cost models rebuilds shared curves from current contents on the next request.
- **Ranking and reporting workflows share one set of equity curves**: `BacktestExecutionResult.getTopStrategies(...)` and `rankTradingStatements(...)` now evaluate each trading record's criteria against one lazily built curve set instead of every criterion building its own; on the same eight-criterion workload ranking runs about 1.3x (DoubleNum) to 1.05x (DecimalNum) faster. Wrap your own multi-criterion loops in `EquityCurveCache.evaluate(series, record, () -> ...)` for the same effect, and let custom criteria share curves by fetching them through `EquityCurveCache.cashFlow(...)`/`cumulativePnL(...)`. Results are unchanged; shared curves are read-only.

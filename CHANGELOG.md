...
### Added
- **Transparent series-scoped indicator sharing**: Standard bar series now share calculation state across structurally equivalent deterministic indicators, including ordinary constructor-built graphs, while preserving distinct indicator object identity. `BarSeries.indicators()` provides concise canonical factories, custom deterministic indicators can opt in with `identityOf(...)`, historical replacements and clears invalidate shared state through a series epoch, and stateful indicators remain isolated. In non-toy BTC/ETH 5-JVM fleet sweeps (same test harness used for the paired CF migration), duplicate indicator graphs are evaluated once per index path, with retained heap around **31.4 MB** and sweep latency consistently inside practical production windows without manual cache wiring—an order-of-magnitude reduction in duplicate work versus pre-sharing workflows.

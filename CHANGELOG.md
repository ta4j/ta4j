...
### Added
- **Constructor-first portfolio analytics**: Added `PortfolioSeries`, `PortfolioAllocation`, and `PortfolioSeriesManager` for strictly aligned multi-asset series, long-only target allocations, scheduled cost-aware rebalancing, immutable snapshots, and portfolio value-series export. The same aligned data supports price/simple-return/log-return correlation matrices, SciPy-compatible complete-linkage hierarchies, and Num-native long-only minimum-variance optimization with an optional maximum asset weight.
- **Adjusted portfolio reports**: Added yfinance-style adjusted Yahoo OHLC loading and an anonymous `DiversifiedPortfolioAnalysis` example that writes annotated JFreeChart heatmaps, dendrograms, equal/pure/capped minimum-variance allocations, HTML, XLSX, and an external-AI prompt without making model calls.

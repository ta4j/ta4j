## Unreleased
- **Futures review round 37**: a funding event id that disagrees with an already processed flow is rejected before any due schedule is applied, so a conflicting call no longer publishes unrelated funding before it fails; and adopted positions plus the open-position cost basis keep the entry trade's retained basis whenever no fill is excluded, deriving it from retained fills only when a deferred or as-of filter drops fills.

...
- **Backtests now fail loudly when the series changes under them.** `BacktestExecutor` (every `executeWithRuntimeReport`, `executeAndKeepTopK` and `executeWithWalkForward` overload) and `StrategyWalkForwardExecutor` capture the series window when they start, run every strategy over exactly that window, and throw `IllegalStateException` if bars inside it are replaced, updated in place (including the forming last bar), or evicted by retention before the run finishes. Appending bars beyond the window is still fine: strategies are evaluated and filled against the captured window, so appended bars are ignored. A rolling `ConcurrentBarSeries` with a maximum bar count evicts on every append, so a backtest over it fails whenever a bar arrives mid-run. To backtest live data, pause writes during the run, or build the strategies on a stable copy such as `series.getSubSeries(series.getBeginIndex(), series.getEndIndex())`, which also leaves out the forming bar. Previously such changes were silently mixed into results.
- **Mark-to-market holding costs accrue per held bar.** `CashFlow`, `CumulativePnL` and `Returns` now charge holding cost accrued since entry at every mark, so the mark-to-market curve ends at the realized value. Previously each mark and the exit subtracted a single period's average cost, so results change for any series with a holding-cost model, along with the criteria built on these curves.
- **A position opened and closed on the first bar now has a return there.** `Returns` reports the realized return in the first retained slot instead of the no-prior-close placeholder, and `getSize()` counts it.
...
- **Backtests run on the caller's live series without holding its lock**: `BarSeriesManager`, position sizing contexts, and the Elliott/Wyckoff facades use the caller's series, with its begin index, instead of an index-resetting copy, so signals and fills read the same bars. Executions report their window as an immutable, index-preserving `barSeries()`, statements and criteria are computed on that window, and parallel scheduling now applies to every series type, so a live `ConcurrentBarSeries` keeps accepting writes and serving readers while a backtest runs. `executeWithWalkForward` shares one window across its backtest and walk-forward phases.
- **`BarSeries.withReadLock` and curve windows are available through the core interfaces**: `ConcurrentBarSeries.withReadLock` is now declared on `BarSeries` (running the action directly for other series) and delegated by indicator bar-series views, for short, bar-only reads; its Javadoc explains why indicators must not be evaluated inside it. `PerformanceIndicator` declares `getBeginIndex()`/`getEndIndex()`, which `CashFlow`, `CumulativePnL` and `Returns` implement with the analysis window they captured.
...
- **Deadlocks and stalled writers between backtests, indicator readers and live feeds**: backtests, walk-forward runs, and the Elliott/Wyckoff indicators and facades no longer evaluate indicators while holding the series read lock. Indicator caches take their own lock before reading bars, so the reverse order could deadlock a strategy run, another thread reading the same indicator, and a waiting feed writer. `FractalSwingDetector` likewise replays outside the read lock instead of blocking writers for the whole replay, then re-checks the series history, retries a replay the series changed under, and throws `IllegalStateException` after 8 attempts rather than return pivots from mixed bars.
- **Index-addressed strategies fire on offset and rolling series**: execution helpers and results no longer copy the series into an index-resetting copy, so strategies and rules keyed to absolute indexes, including ranges ending at `Integer.MAX_VALUE`, see the indexes they were built for.
- **Analyses price exits beyond the logical window end and respect explicit record bounds**: `CashFlow`, `CumulativePnL`, `Returns` and `InvestedInterval` value exits that landed in retained raw bars past the logical end at their real fill prices, while `TradingRecord.getEndIndex(BarSeries)` stays the logical bound for benchmarks. Their captured window ends at the record's end (or that trailing exit), so Calmar and Omega no longer score the flat tail after an explicitly bounded record, and absolute drawdown now includes trailing exits like relative drawdown does.

...
### Added
- **Script ta4j workflows without writing throwaway Java mains**: added the new `ta4j-cli` module with bounded `strategy backtest`, `strategy walk-forward`, `strategy sweep`, `indicator test`, `rule test`, `performance run`, and `performance compare` commands for local OHLCV datasets, reusable optimization experiments, deterministic JSON output, and optional headless chart export.

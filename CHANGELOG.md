...
- **Support/resistance trendlines configuration**: Scoring no longer includes a "contains current price" bonus category; weights now sum to 1.0 across touch/extreme/outside/proximity/recency. Tolerance is configurable via percentage, absolute, or tick-size settings, and the active segment (anchors, slope/intercept, touches/outside, score, and containment flag) is now exposed for downstream consumers.

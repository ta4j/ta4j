...
### Added
- **CF-289: Forecast predictions for forward price estimates**: Added a ta4j-core forecast indicator layer with `PredictionSnapshot.Forecast` summaries, reusable `EWMAIndicator` and `ForecastStateIndicator` building blocks, deterministic Monte Carlo cumulative-return forecasts, and projection methods for close-price quantiles, medians, means, and standard deviations as regular `Indicator<Num>` values.

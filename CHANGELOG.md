...
- **Criteria equity curves compute in one sweep and reuse cached state**: `CashFlow` and `CumulativePnL` build their full curve with a single forward pass per construction, `InvestedInterval` reads invested state directly from the trading record with a stamp-guarded cold-miss fallback, and `BaseTradingRecord` caches closed/open position lookups keyed by its modification count, so repeated criterion evaluation no longer re-derives the same state. `CashFlow`, `CumulativePnL`, and `Returns` now capture a single construction-time series snapshot and expose it through `getBarSeries()` instead of tracking live series replacement; criteria constructed against a series that is later mutated keep evaluating against that snapshot. `ExcessReturns` short-circuits risk-free growth computation when the annual risk-free rate is zero.
- **Batch criterion evaluation shares one equity bundle**: new opt-in facade `CriteriaEvaluation.evaluateAll(series, tradingRecord, criteria)` evaluates several criteria in one pass over a shared `EquityBundle`, which lazily memoizes one `CashFlow`/`CumulativePnL`/`InvestedInterval` instance per (equity curve mode, open position handling) configuration key. Criteria advertise participation through the `EquityBundleAware` interface (`MaximumDrawdownCriterion`, `MaximumDrawdownBarLengthCriterion`, `MaximumAbsoluteDrawdownCriterion`, `MonteCarloMaximumDrawdownCriterion`, `ReturnOverMaxDrawdownCriterion`, `CalmarRatioCriterion`, `SharpeRatioCriterion`, `SortinoRatioCriterion` so far); non-aware criteria fall back to their existing `calculate`. A new `ExcessReturns` constructor accepts existing `InvestedInterval`/`CashFlow` instances so ratio criteria consume the bundle's curves without copying. Results are keyed by criterion instance in argument order; duplicate instances collapse onto the first occurrence.

...
- **Portfolio correlation matrices**: Added `PortfolioCorrelations` for rolling price, simple-return, and log-return correlation matrices over an `AlignedPortfolioSeries`, reusing ta4j statistics indicators while preserving the portfolio feature's strict common-end-time timeline; the examples module includes the AssetCorrelations sample ticker universes.

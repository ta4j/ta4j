...
- **Static target-weight portfolio backtesting (`CF-148`)**: New `org.ta4j.core.portfolio` package mirroring the single-series workflow: `PortfolioSeries` aligns one or more named bar series by common end time and owns detached bar copies; `PortfolioAllocation` holds long-only target weights (the remainder is cash, `Map.of("SPY", 0.6)` works); `PortfolioSeriesManager.run(...)` executes buy-and-hold by default or any `RebalancePolicy` (`everyBar`, `everyNthBar`, `onIndexes`, calendar `firstBarOf`, combinable with `or`), solving targets against post-cost value so cash never goes negative and fixed fees never churn. Each immutable `PortfolioSnapshot` reports holdings, achieved asset/cash weights, traded notional versus turnover ratio, costs, and a `COMPLETED`/`PARTIAL`/`SKIPPED` rebalance outcome; `PortfolioExecutionResult` summarizes the run and exports a value series whose first bar is the initial capital, so existing criteria reconcile with its total return. See `StaticPortfolioBacktest`.

...
- **Indicator-family analysis foundations for CF-21**: Added deterministic manifest-driven family analysis primitives (`IndicatorFamilyManifest`, `IndicatorFamilyAnalysisEngine`, `IndicatorFamilyCatalog`, `IndicatorFamilyAnalysisResult`) plus family-aware selector support and usage guidance, and hardened `CorrelationCoefficientIndicator` serialization by reconstructing derived covariance/variance fields from serialized source indicators.

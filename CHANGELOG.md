...
- **Transparent forecast acceleration in existing backtests (`CF-336`)**: `BarSeriesManager` can now batch eligible `DoubleNum` Monte Carlo price forecasts through lazy Metal or CUDA providers from `ta4j-cli` while preserving `Indicator#getValue(int)`, strategy, executor, and trading-record APIs. The production switch is intentionally just omitted/`off` or `auto`; unsupported work, unqualified hardware, stale data, memory limits, and native failures retain complete scalar results. Optional native classifiers cover macOS arm64 Metal plus Windows/Linux x86_64 CUDA packaging, with M5 Max Metal measured at 2.23x end-to-end on the checked 4,096-bar backtest and Linux hardware qualification still open. New Linux x86_64/aarch64 OpenCL classifiers provide vendor-neutral GPU/hardware acceleration (NVIDIA, AMD, Intel, or CPU ICDs), correctness-qualified inside a PoCL Docker matrix on both architectures; automatic OpenCL selection is gated to FP64 GPU devices at a 16,777,216 path-step floor pending real-device speedup measurement. Seeded Monte Carlo simulations now use a versioned per-path generator shared by scalar and native execution; results remain deterministic for a fixed seed but intentionally differ from the pre-0.23.1 `SplittableRandom` sequence.
...
### Fixed
- **CLI performance artifacts and named-rule scans are more robust**: performance runs now canonicalize bar-count inputs before writing comparison-ready artifacts, bound git metadata lookup so a stalled subprocess returns `unknown` instead of hanging the run, and named-rule package scans skip invalid or conflicting discovered rule classes while continuing to register valid ones.

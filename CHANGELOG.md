...
- **Reusable performance experiment runner**: `PerformanceExperimentRunner` can run named examples-layer performance experiments, capture `performance.json` plus Markdown summaries with hashed host/JVM metadata, and compare two git refs through `scripts/benchmark-performance-experiment.sh`. The first packaged workload covers `KalmanFilterIndicator` access patterns.

...
### Added
- **Causal swing detection for sharp, rounded, and multi-scale turns**: Added `SlopeChangeSwingDetector` for sustained rolling-regression reversals, including a window-only convenience constructor with conservative defaults, and tolerant `SwingDetectors.multiScale(...)` consensus, while ZigZag detection now locates pivots from intrabar highs/lows, confirms reversals from closes with pivot-anchored thresholds, and resolves fractal plateaus to one deterministic midpoint.

...
- **BarSeries snapshots preserve the series begin index and raw-to-logical offset**: `BarSeries` now exposes a polymorphic `snapshot()` returning a detached `BaseBarSeries` with the same name, raw bar data, begin/end index range, `removedBarsCount`, num factory, and maximum bar count, and every execution/analysis helper (`BarSeriesManager`, `BacktestExecutionResult`, `StrategyWalkForwardExecutionResult`, position sizing, `CashFlow`, `CumulativePnL`, `Returns`, and the Elliott/Wyckoff facades) delegates to it. The snapshot is rebuilt from public accessors, so subclass overrides that advertise a shifted logical view are honored, and `ConcurrentBarSeries` observes the source atomically under a single read lock. Series constructed with a non-zero begin index (for example, a terminal `Integer.MAX_VALUE` range) keep their logical index range and index-addressed strategies and rules fire as expected instead of silently producing no positions. The analysis curves (`CashFlow`, `CumulativePnL`, `Returns`, `InvestedInterval`) now store values offset-compactly relative to the window start instead of allocating absolute `endIndex + 1` slots, so terminal and offset series neither overflow `int` sizing nor throw on addressable indexes; out-of-window reads resolve to documented neutral values (zero, one, or NaN) instead of throwing.

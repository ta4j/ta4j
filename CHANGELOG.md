...
### Changed
- **Elliott anchor replay is harness-only**: Long BTC macro-cycle replay now lives behind `ElliottWaveAnchorCalibrationHarness` instead of a JUnit tag, and the default/tagged Maven lanes document only runnable test suites while warning that full anchor calibration can run for 8+ hours.
...
- **SpotBugs remediation**: Cleared targeted baseline entries for realtime bar equality, serializable bar builder factories, walk-forward calibration map iteration, charting/cache path guards, forced garbage collection in backtest batching and benchmark harness flows, `DoubleNumFactory` singleton serialization, descriptor-only chain links, `DoubleNum`/`NaN` comparisons, performance-report ordering, lower-camel criteria factories, `BaseBar` time resolution nullness, and obsolete Elliott macro demo private bridges.

...
- **Release prep now opens cleanup issues for removal-ready APIs**: `prepare-release.yml` now runs `scan-removal-ready-deprecations.py` after bumping to the next snapshot, groups matching `@Deprecated(forRemoval = true)` symbols by source file, and creates or refreshes deduplicated GitHub issues plus an artifact report so maintainers can clear scheduled removals without a manual repo sweep.

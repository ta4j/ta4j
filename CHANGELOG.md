...
### Added
- **Bayesian online change-point return state**: Added `OnlineChangePointForecastStateIndicator` with constructor-first defaults, canonical constant-hazard run-length inference, Normal-Inverse-Gamma/Student-t updates in log space, deterministic typed posterior summaries, reset-aware warm-up, and a fixed feature schema for analog composition. Its operator-facing `recentChangeProbability` reports posterior mass over a configurable recent run-length window rather than mislabeling the constant reset hazard as a responsive signal.

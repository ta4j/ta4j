...
### Added
- **Optional indicator batch acceleration boundary (`CF-336`)**: Added a dependency-free explicit batch-evaluation contract in `ta4j-core` plus an optional `ta4j-accelerator` module for adapter/provider planning, typed diagnostics, required-provider failures, and CPU fallback. AUTO/HYBRID honor qualified end-to-end speedup thresholds, provider probe/execution failures remain CPU-safe, and the first adapters cover a CPU-planned Close/SMA control graph and the Monte Carlo price forecast family. An opt-in `cuda-windows-x86_64` classifier now provides the CUDA 13.3 JNI backend qualified for Windows x86_64 and compute capability 12.0; explicit CUDA executes deterministic FP64 Monte Carlo forecasts while AUTO/HYBRID remain CPU-planned until a stable crossover is demonstrated. Metal execution and Linux CUDA packaging remain separate qualification lanes.

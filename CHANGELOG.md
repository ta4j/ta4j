...
- **BarSeries snapshots preserve the series begin index and raw-to-logical offset**: `BarSeriesManager.snapshotSeries()` and the backtest result snapshots now delegate to `BaseBarSeries.copyOf(...)`, which preserves the begin/end index range, `removedBarsCount`, and raw bar data of the source series (including leading orphans and trailing constrained bars), so series constructed with a non-zero begin index (for example, a terminal `Integer.MAX_VALUE` range) keep their logical index range and index-addressed strategies and rules fire as expected instead of silently producing no positions. Concurrent series are copied atomically under a single read lock.

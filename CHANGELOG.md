...
- **Candle geometry primitives**: Added `CandleBodyIndicator` (body magnitude, `|close - open|`) and `CandleRangeIndicator` (`high - low`) to the candlestick package, so pattern indicators can measure body size and total range without abusing the signed legacy `RealBodyIndicator`. All internal candle-pattern indicators (doji, marubozu, hammer family, kickers, piercing/dark-cloud family, morning/evening stars) now use the magnitude-based body primitive, preserving their exact published behavior.
- **Stricter OHLC bar validation**: `BaseBar` construction now rejects candles whose high is below the open or close, or whose low is above the open or close (in addition to the existing high-above-low check), guaranteeing the non-negativity invariants of the upper shadow, lower shadow, body, and range geometry indicators. Null prices remain tolerated and `NaN` comparisons pass validation.
- **`RealBodyIndicator` (signed real body)**: The legacy signed `close - open` body indicator is deprecated in favor of the magnitude-based `CandleBodyIndicator` plus `Bar#isBullish()`/`Bar#isBearish()` for direction. The signed value is a direction and magnitude conflated in one number, which has caused subtle sign bugs in pattern logic. `RealBodyIndicator` retains its existing behavior for backward compatibility.
- **Adaptive candle thresholds consolidate pattern sizing**: Added a package-private `CandleThresholdSupport` to the candlestick package that computes causal prior-window baselines (`SMAIndicator` of the preceding `averagePeriod` bars shifted by `PreviousValueIndicator`) for body and range measurements and exposes long/short-body, doji, long/short-shadow, and near classifiers with one documented recommended profile, so named patterns no longer invent body or shadow threshold machinery independently.
- **Single-candle patterns use explicit adaptive morphology**: Doji, bullish/bearish marubozu, hammer, hanging man, inverted hammer, and shooting star now use causal prior-window body/range thresholds with documented strict and inclusive boundaries. They no longer embed ADX trend gates: compose any trend context explicitly in a strategy. Migrate removed ratio/configuration constructors to the new `(series, averagePeriod)` forms, or `(series, averagePeriod, rangeFactor)` for doji.
- **Candle threshold support interned per series and period**: `CandleThresholdSupport.forSeries(series, averagePeriod)` now interns one support per distinct pair through a weakly-keyed identity table, so every single-candle pattern over the same series shares one cached set of body/range/shadow primitives instead of recomputing baseline averages per instance; construction-time validation is unchanged.

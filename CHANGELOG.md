...
- **Slow tagged tests are easier to run when you actually want them**: `benchmark`, `integration`, and `slow` JUnit tags now each have a dedicated GitHub Actions workflow with manual dispatch plus independently gated daily, weekly, or monthly schedule slots controlled by repository variables, so long-running suites stay out of push CI without becoming hard to exercise.

...
- **Advanced lead/lag, DTW, and event-dependence analysis (`CF-454`)**: Added `LeadLagCorrelationAnalyzer`, which scans an inclusive lag range and returns a full `LagCorrelationProfile` with every best lag and one deterministic selected lag under the existing `LaggedCorrelationIndicator` sign convention; `DynamicTimeWarpingDistanceIndicator`, a bounded two-row-DP shape distance with z-score or raw normalization, absolute or squared local distance, an explicit Sakoe–Chiba band (unconstrained opt-in), and exact path-length normalization; and `EventMutualInformationEvaluator`, which measures how much a continuous predictor reduces uncertainty about a sparse Boolean event in an explicit current-or-future bar window, reporting raw and normalized MI in nats, target entropy, prevalence, and effective bin count. Event-MI target windows never cross the evaluation partition boundary, and equal-frequency binning never splits tied predictor values.

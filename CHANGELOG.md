...
### Changed
- **Finite `Num` validation is reusable across indicators**: Added `Num.isFinite(...)` for indicator-safe checks that reject null, NaN, and primitive-backed infinities without misclassifying finite high-precision `DecimalNum` values whose `doubleValue()` overflows; `IndicatorUtils.isInvalid(...)` now delegates to this shared contract.

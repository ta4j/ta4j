...
- **BarSeriesManager snapshots preserve the series begin index**: `BarSeriesManager.snapshotSeries()` now rebuilds the managed series with `withBeginIndex(...)`, so series constructed with a non-zero begin index (for example, a terminal `Integer.MAX_VALUE` range) keep their logical index range and index-addressed strategies and rules fire as expected instead of silently producing no positions.

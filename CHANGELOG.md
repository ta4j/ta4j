...
- **GPU acceleration widens automatic selection (`CF-107`)**: CUDA joins `ta4j.acceleration=auto` on compute-capability 12.0 GPUs for workloads above ~16.7M Monte Carlo samples (measured up to 2.28x on an RTX 5090), and Metal automatic selection accepts any Apple GPU instead of only the M5 Max.

...
- **Candle indicator foundation: geometry primitives and stricter OHLC validation**: Added `CandleBodyIndicator` (body magnitude, `|close - open|`) and `CandleRangeIndicator` (`high - low`) to the candlestick package, so pattern indicators measure body size and total range without abusing the signed legacy `RealBodyIndicator`; all internal candle-pattern indicators (doji, marubozu, hammer family, kickers, piercing/dark-cloud family, morning/evening stars) now use the magnitude-based body primitive, preserving their exact published behavior. `BaseBar` construction now also rejects candles whose high is below the open or close or whose low is above the open or close, guaranteeing the non-negativity invariants of the upper shadow, lower shadow, body, and range geometry indicators (null prices remain tolerated and `NaN` comparisons pass). The legacy signed `RealBodyIndicator` is deprecated in favor of `CandleBodyIndicator` plus `Bar#isBullish()`/`Bar#isBearish()` for direction and retains its existing behavior for backward compatibility.

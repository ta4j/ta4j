...
### Changed
- **AI release scheduling now uses direct OpenAI Responses API inference**: `release-scheduler.yml` no longer depends on the retired GitHub Models service; it preflights the exact `gpt-5.6-luna` model, sends the existing SemVer dossier with reasoning effort `high`, and records provider-aware request, response, and failure diagnostics while preserving probe/full/skip and dry-run safety.

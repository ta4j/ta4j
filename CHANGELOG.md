...
- **Transparent forecast acceleration preserves existing backtest APIs (`CF-336`)**: `BarSeriesManager` can batch eligible `DoubleNum` Monte Carlo price forecasts through the optional, ServiceLoader-based `org.ta4j.core.acceleration.AccelerationRuntime` SPI while preserving `Indicator#getValue(int)`, strategy, executor, and trading-record contracts. Acceleration is strictly opt-in (`-Dta4j.acceleration=auto`, explicit per-path RNG selection) and always falls back to complete scalar results for unsupported work, unqualified hardware, stale data, memory limits, or native failures.
- **Acceleration moves to an operation-level kernel ABI (`CF-336`)**: providers no longer probe concrete indicators. Core-internal planners lower eligible Monte Carlo shock-path batches to immutable versioned `KernelRequest` primitives; providers expose side-effect-free `assess` plus `execute`, and the core owns selection, snapshot validation, scalar fallback, and decoding. The Metal provider serves approximate requests only under explicit opt-in, CUDA/OpenCL decline sample-output versions their reduced-row native lane cannot serve, and `NamedAssetRegistry.aliases(kind)` returns as the CLI catalog's read-only discovery view.
- **Acceleration certification bounds tolerance floors, planner memory, and provider qualification (`CF-336`)**: approximate Metal requests decline below the fp32 lane's certified relative-tolerance floor (1e-6 per horizon step) instead of running uncertifiable accuracy, Monte Carlo planners decline batches whose peak device estimate exceeds the `ta4j.acceleration.maxDeviceBytes` budget before materializing inputs, and the Metal provider derives the device family from the CPU brand so qualified cost rows are reached on cold starts. `forecast run` bounds CSV data files to 512 MiB like its JSON counterpart, `--capital`/`--stake-amount` limits compare at the series numeric factory's precision, `rule test` clamps normalized indicator windows so serialized negative windows cannot mask rolling-window work, and `performance compare` fingerprints JVM options so artifacts captured under different JVM configurations are refused.
- **Named-rule scans tolerate invalid or conflicting discovered rule classes**: named-rule package scans skip invalid or conflicting discovered rule classes while continuing to register valid ones.
...
- **CLI batch executions isolate failures and bound expensive searches**: `strategy backtest`, `strategy walk-forward`, and `rule test` keep healthy strategies and folds when siblings fail, reporting `failedStrategies` / `failedStrategyCount` and `failedFoldCount` / `foldFailures` additively with `partial` or `error` envelope statuses instead of discarding the batch. `strategy sweep` rejects unknown `--param-grid` keys and grids expanding past 10,000 candidates, `forecast run` rejects Monte Carlo work beyond 10,000,000 sample-horizon units, and `performance compare` reports a gated candidate as envelope `error` while the nested comparison keeps its `regression` status. The Metal acceleration provider reports itself unavailable until approximate fp32 results are opted in via `-Dta4j.cli.acceleration.metal.approximate=true`, and the release manifest now lists the assembly fat jar. Batches whose walk-forward folds all fail record a per-strategy failure instead of an invalid-geometry usage error, unreadable structured input files exit with the I/O status (74) rather than a usage error, and same-file collision checks now apply only to writable artifacts (`--output`, `--chart`) so entry and exit rules may share one JSON file; `performance compare` likewise rejects an `--output-dir` that aliases either compared input directory. OpenCL acceleration device selection now prefers an FP64 GPU with at least 2 GiB of global memory before falling back to the first FP64 device, mirroring the qualification floor the provider model applies to predicted speedups. The forecast work ceiling now includes conformal calibration history and bounds analog horizon-by-lookback work, performance comparison gates timings on recorded OS versions, the OpenCL benchmark creates its report directory, and the CLI installs a quiet Log4j configuration at startup so stdout carries only command output while operational logs stay on stderr. Change-point forecast state is bounded by history-by-maximum-run-length work before any projection runs, CSV loading discards partial rows when a file cannot be fully read, and output artifact alias detection follows dangling symlink leaves so a not-yet-created target cannot bypass the same-file collision checks. Unreadable or missing market data files now surface as io failures with exit code 74 instead of usage errors, and malformed performance comparison artifacts are reported as usage errors identifying the offending file. Data files that fail mid-read now keep the I/O exit status (74) instead of degrading to a usage error, comparison artifacts with wrong-shaped fields are rejected up front naming the file, `strategy sweep` bounds candidate-grid-by-bar work at 100,000,000 evaluations, and the Maven Central metadata validation gate covers ta4j-cli.
- **CLI structured inputs fail loud instead of degrading silently**: JSON data files are re-validated after decoding so candles the adapter had to skip abort the run instead of silently shrinking the dataset, `--timeframe` aggregation rejects source bars with non-uniform periods instead of mis-bucketing them, and `RsiThresholdRule` rejects non-finite thresholds, which previously produced a rule that could never match.
- **Named-rule traces report the compact label**: named rules that delegate to an inner rule (for example `ClosePriceCrossedMovingAverageRule`) now trace the outer result under the compact label with the delegate as a child (`path=root.delegate`) instead of logging only the raw delegate rule.
- **Rough-volatility horizons stop at a memory-aware bound**: `RoughVolatilityForecastStateIndicator` rejects horizons above 100,000 steps so the per-state cumulative variance term structure can no longer exhaust the heap, and `forecast run` validates projection work before evaluating the state, rejecting over-budget Monte Carlo or analog requests before any state allocation.
- **CLI indicator tests resolve structured input exactly once**: `indicator test` resolves `--indicator` / `--indicator-json-file` input once and hands the resolved indicator to the temporary strategy builder, so the input file can no longer be re-read (and potentially changed) between validation and execution.
- **OpenCL acceleration ceilings count both native sort buffers**: the OpenCL provider's memory estimate now accounts for the padded host buffer and the device bitonic-sort buffer, so iteration counts near powers of two can no longer fit an under-estimated allocation.
- **Metal acceleration ceilings account for normalization and sort copies**: the Metal provider's per-decision memory estimate now includes the 68 bytes `Forecast.ofSamples` needs while normalizing and sorting samples (terminal-price defensive copies, the original and normalized `Num` objects, and the live reference arrays), so a chunk can no longer be admitted at a memory ceiling it cannot survive.
- **CLI backtests and walk-forward runs bound batch evaluation work**: `strategy backtest` and `strategy walk-forward` reject batches exceeding 100,000,000 bar-strategy evaluations before execution starts. Backtests count strategies × bars; walk-forward derives the evaluated bars from the actual split geometry (the full-series backtest pass plus every fold's and holdout's test-bar range) instead of estimating passes from the fold count.
- **Named rules and strategies share one registry implementation**: the package-scanning, class-loading, label-validation, registration, and test-reset machinery now lives in a single `NamedComponentRegistry`, so loader, concurrency, and validation fixes apply to both component families at once. `NamedStrategy` lookups now initialize the default registry like `NamedRule` lookups, and both families reject underscore-delineated class names. CLI strategy JSON deserialization initializes the `ta4jexamples.strategies` registry before deserializing, so `NamedStrategy` JSON labels resolve in fresh processes.
- **CLI fold failures carry their root cause and batch JSON I/O errors stay I/O errors**: walk-forward reports include a sanitized `cause` field on `foldFailures` entries when the fold threw, the all-fold-failed summary appends the cause, and an unreadable `--strategies-json-file` now fails the command as an I/O error instead of degrading into invalid-strategy entries.
## 0.24.1 (2026-08-10)
...
- **Script ta4j workflows without writing throwaway Java mains**: added the new `ta4j-cli` module with bounded strategy, indicator, rule, forecast, and performance commands for local OHLCV datasets. Compact and lossless serialization v2 inputs, dynamic balance/Kelly sizing, side-aware borrowing costs, Monte Carlo/analog/conformal forecasts, versioned reproducible JSON, fail-safe batch execution, stdin pipelines, structured errors, live catalog discovery, generated completion, and optional headless chart export keep both interactive and automated runs on current ta4j APIs.
- **Concurrent performance comparisons stay isolated**: the `ta4j-cli` benchmark helper now uses collision-proof default output directories and an invocation-scoped Maven local repository, preventing overlapping agent or CI runs from sharing mutable SNAPSHOT artifacts.
...
### Fixed
- **Named-rule scans tolerate invalid or conflicting discovered rule classes**: named-rule package scans skip invalid or conflicting discovered rule classes while continuing to register valid ones.
- **CLI performance artifacts are more robust**: performance runs now canonicalize bar-count inputs before writing comparison-ready artifacts, and bound git metadata lookup so a stalled subprocess returns `unknown` instead of hanging the run.
- **Named drawdown criteria no longer resolve through deprecated compatibility classes**: `MaximumDrawdown` and `ReturnOverMaxDrawdown` shorthand now instantiate their current `criteria.drawdown` implementations without deprecation warnings.

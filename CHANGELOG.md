...
- **Indicator-family analysis foundations for CF-21**: Added deterministic manifest-driven family analysis primitives (`IndicatorFamilyManifest`, `IndicatorFamilyAnalysisEngine`, `IndicatorFamilyCatalog`, `IndicatorFamilyAnalysisResult`) with catalog-level family-aware selection and usage guidance, and hardened `CorrelationCoefficientIndicator` descriptor/JSON reconstruction from serialized source indicators.

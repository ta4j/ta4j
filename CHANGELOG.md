...
- **CLI workflows (`CF-336`)**: Added the `ta4j-cli` module (`ta4j-cli/README.md`) for bounded local strategy backtests, walk-forwards, sweeps, indicator and rule tests, forecasts, and performance experiments over CSV or JSON bars, with text or JSON output and structured errors. `--capital` and `--stake-amount` are cash amounts in every sizing mode. Error envelopes (`usage` exit 2, `io` exit 74, `execution`/`software` exit 1) keep the offending input and ta4j's reason, and `--error-format json` works anywhere on the command line. Runs whose every member failed also report on stderr when the response goes to `--output`. CSV input is daily `yyyy-MM-dd` bars with the failing row named, and `--timeframe` only aggregates to coarser bars. Sweeps skip `fast >= slow` pairs (`skippedCandidateCount`), mixed long/short backtests report in input order, and progress prints `completed/total`.
- CLI work bounds are enforced before execution for every reporting command: Monte Carlo drawdown criteria are charged for the backtest and each fold, strategy batches pay for rolling-window scanners reachable from their rules (including serialized strategies), and indicator tests bound window scans before evaluating the indicator. Walk-forward runs rebuild serialized strategy state for the full backtest and each fold and score each fold once.
- `performance compare` rejects a candidate directory that aliases the base directory and names mismatched experiment, host, or JVM fields as usage errors before creating output; empty experiment grids fail instead of passing. `performance run` defaults to `./ta4j-performance/<experiment>/<timestamp>`, and the benchmark script fails when `exec:java` produces no artifacts.
- **Named rules**: Added `org.ta4j.core.rules.named.NamedRule`, compact-label rules registered by class name and rebuilt from labels such as `RsiThresholdRule_BELOW_14_30`, mirroring `NamedStrategy`; the CLI resolves `--entry-rule`/`--exit-rule` labels through it. Subclasses call `super(TheirClass.class, parameters...)`, and `NamedRule` builds and validates the label before the rule exists; two scanned classes sharing a simple name fail the package scan instead of resolving by classpath order.
- **Optional native Monte Carlo providers (`CF-336`)**: The `ta4j-acceleration` module ships CUDA, OpenCL, and Metal providers for the versioned shock-path kernel ABI. They consume a shared returns buffer and emit raw cumulative log-returns that core decodes, assess before native initialization, honor explicit library overrides, and reject infeasible memory limits early. Approximate requests are admitted only when a condition-aware error bound over the actual inputs and lane precision certifies the requested tolerance. Automatic selection requires a predicted 1.5x end-to-end speedup from a measured qualification row, so unqualified devices stay scalar with a `CPU_FASTER` diagnostic.

...
- **SpotBugs remediation**: Cleared targeted baseline entries for realtime bar equality, serializable bar builder factories, walk-forward calibration map iteration, charting/cache path guards, forced garbage collection in backtest batching and benchmark harness flows, `DoubleNumFactory` singleton serialization, descriptor-only chain links, `DoubleNum`/`NaN` comparisons, performance-report ordering, lower-camel criteria factories, constructor-safety validation paths, backtest execution, support/resistance, average, MACD/PPO, edge/momentum helper, scoring/compression, oscillator, and Elliott scenario indicators, and triggered/Elliott rule constructor validation, Elliott suite analysis-result ownership, `BaseBar` time resolution nullness, and obsolete Elliott macro demo private bridges.

...
### Added
- **CF-289: Forecast distributions for forward price estimates**: Added a ta4j-core forecast indicator layer with immutable `ForecastDistribution` summaries, log-return transforms, EWMA return state, deterministic Monte Carlo cumulative-return forecasts, and distribution projection methods for close-price quantiles, medians, means, and standard deviations as regular `Indicator<Num>` values.

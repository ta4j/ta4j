...
- **Indicator-family analysis now starts from the indicators you already built**: `IndicatorFamilyManager` groups a named map of live indicators over a `BarSeries`, lets you tune the similarity threshold and rolling correlation window, and returns compact `IndicatorFamilyResult` families plus pairwise similarity scores. The examples module now includes an S&P 500 walkthrough that shows how threshold changes split broad indicator families into tighter substitutes.

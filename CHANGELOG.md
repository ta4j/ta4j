...
### Changed
- **Build and test dependencies were refreshed**: The parent POM now pins SpotBugs `4.9.8.3`, JaCoCo `0.8.14`, Gson `2.14.0`, JUnit Jupiter `6.1.0`, Mockito `5.23.0`, and SLF4J `2.0.18`.
- **Concurrent bar-series tests are easier to read in logs**: `ConcurrentBarSeriesTest` and `ConcurrentBarSeriesBuilderTest` now give every built series a descriptive name derived from its test method, concurrent harnesses log caught exceptions through SLF4J instead of printing stack traces, and `ta4j-core` test Log4j configuration suppresses intentional `TimeBarBuilder` missing-bar WARNs while keeping explicit gap-logging assertions in `TimeBarBuilderTest`.

...
### Added
- **Script ta4j workflows without writing throwaway Java mains**: added the new `ta4j-cli` module with bounded strategy, indicator, rule, forecast, and performance commands for local OHLCV datasets. Compact serialization v2 expressions and JSON, named criteria, dynamic balance/Kelly sizing, side-aware borrowing costs, deterministic forecast projections with support provenance, machine-readable output, and optional headless chart export keep both interactive and automated runs on current ta4j APIs.

...
- **Lead/lag and event-dependence analysis hardened (`CF-454`)**: Pearson correlation rescales its window by the largest absolute value before averaging, so extreme-but-finite values can no longer overflow the mean and turn an exactly correlated window undefined; `Profile` rejects points whose sample count is neither 0 nor the profile's bar count; event-mutual-information evaluation returns an undefined result for empty series and for target-window spans at the JVM array ceiling instead of throwing; and the result invariant now pairs a constant target with NaN normalized mutual information and a non-constant target with a defined value.

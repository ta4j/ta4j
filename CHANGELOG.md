...
- **CUDA acceleration gains an end-to-end Windows benchmark (`CF-107`)**: `CudaBacktestBenchmarkTest` mirrors the Metal and OpenCL transparent-backtest methodology (4,096-bar synthetic series, 256 forecast decisions, 2,048 Monte Carlo paths, horizon 32) with exact trade-parity assertions and JSON reports under `.agents/benchmarks/cf-336-validation/`, so Windows/CUDA hosts can record evidence comparable with recorded Metal numbers. On a reference RTX 5090 host (CUDA 13.3) the canonical workload runs about 1.09x faster end-to-end with `ta4j.acceleration=auto`; provider-level sweeps place the crossover near 16K samples per decision and up to 2.28x at large path/horizon shapes, with the per-decision sort/reduction phase dominating native time.

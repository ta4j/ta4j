...
- **Futures review round 14**: removed the pathological unbounded-precision affordability test from the default unit lane after the bounded 5000-digit convergence test established the supported finite-precision contract; factory Kelly validation now compares decimal inputs without double narrowing; the default-branch required status rule now points at the canonical Build and Verify gate after the retired standalone formatting workflow was consolidated.

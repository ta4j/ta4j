...
- **Composable Monte Carlo technique decorators**: `RecentVolatilityWideningMonteCarloMethod` widens an inner technique's terminal samples by the bounded ratio of recent realized to state volatility, `StudentTScaleMixingMonteCarloMethod` fattens tails with a mean-normalized Student-t scale-mixing factor, and `EnsembleMonteCarloMethod` pools two techniques 50/50 through derived sub-generators. Each decorator draws exclusively from `MonteCarloContext.random()`, composes with any `MonteCarloMethod`, and degrades to an unstable result when the inner technique returns null, miscounts, or emits non-finite samples. `WalkForwardCalibrationBakeoffExample` now builds its composition arms from these core classes. Widening scales around the inner technique's empirical center (dispersion only, never a location shift); the Student-t scale mean is computed in log space so very high degrees of freedom stay finite, and the chi-square denominator is drawn from the exact gamma representation so arbitrarily large degrees of freedom stay constant-time instead of one Gaussian draw per degree; foreign-precision inner samples are normalized through the context's `NumFactory`; and the bake-off example persists per-arm checkpoints incrementally and serializes empty volatility regimes as `null` rather than non-finite numbers.

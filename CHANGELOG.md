...
- **Lot bookkeeping now stays inside `BaseTradingRecord`**: You can still model FIFO, LIFO, average-cost, and specific-id matching exactly the same way, but the standalone `PositionBook` type is now internal. Callers should go through `BaseTradingRecord` and `TradingRecord` for recording fills, reading closed positions, inspecting per-lot open positions, and querying net exposure.

### Changed (Backtest Execution Models and Custom Records)
- **Trade-first fill flow across live and backtests**: You can now drive `BaseTradingRecord#recordFill(...)` with the `Trade` interface directly, while internals still materialize `BaseTrade` and lot bookkeeping as needed, so calling code stays implementation-agnostic and live/backtest behavior stays aligned.

...
- harami and kicker replace their `(series, Num)` configuration constructors with `(series, averagePeriod)` overloads, and dark-cloud-cover and piercing-line replace `(series, Num, double)` with `(series, averagePeriod, penetration)`; engulfing construction is unchanged. `DarkCloudIndicator`/`PiercingIndicator` are deprecated; use `DarkCloudCoverIndicator`/`PiercingLineIndicator`.
...
- **Candle pattern foundation: shared adaptive thresholds**: Single- and two-candle patterns now share one consistent, causal, cached body/shadow sizing model; single-candle patterns no longer apply ADX trend gates.
...
- **Recursive indicators keep their cached values by default**: `RecursiveCachedIndicator` now opts into the head-advance cache exemption, so subclasses that compute each value from its predecessors keep pre-advance values on bounded series; recursive indicators over a fixed trailing window (`VolumeIndicator`, `PearsonCorrelationIndicator`, `NetMomentumIndicator`, pivot points, and the pivot reversal indicators) opt out explicitly and are recomputed like any windowed indicator.
- **Two-candle patterns normalize signed zero in gap, containment, and engulfing comparisons**: dark-cloud-cover, piercing-line, harami, kicker, and engulfing indicators no longer let the zero sign bit decide a match, so numerically equal endpoints behave identically on every `Num` implementation: `DoubleNum` orders `-0.0` below `+0.0`, and the strict gap/exceed checks guard both-zero operands while the inclusive containment and engulfing checks treat both-zero endpoints as equal.

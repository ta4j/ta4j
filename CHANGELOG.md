...
- **Equity-curve-aware net return**: RoMaD now uses equity-curve returns so equity curve modes and open position handling stay consistent.
...
- **Equity curve mode control and open position handling**: Added constructors to select mark-to-market or realized cash-flow calculation and last open position handling for CumulativePnL, Returns, and all drawdown criteria.
...
- **RoMaD single-position return**: Compute net return directly from entry/exit prices to avoid compounded drift in zero-return cases.

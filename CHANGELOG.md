...
### Added
- **Transparent series-scoped indicator sharing**: Standard bar series now share calculation state across structurally equivalent deterministic indicators, including ordinary constructor-built graphs, while preserving distinct indicator object identity. `BarSeries.indicators()` provides concise canonical factories, custom deterministic indicators can opt in with `identityOf(...)`, historical replacements and clears invalidate shared state through a series epoch, and stateful indicators remain isolated. Backtest managers, executors, walk-forward results, and position-sizing contexts expose snapshot-safe read-only series views that still delegate indicator sharing to their retained series snapshots. In non-toy BTC/ETH 5-JVM fleet sweeps (same test harness used for the paired CF migration), duplicate indicator graphs evaluate once per index path with ~18–25x lower compute cost than the former unshared duplicate-graph path and roughly 20x less retained heap, finishing around **31.4 MB** while staying within the former explicit-sharing baseline.

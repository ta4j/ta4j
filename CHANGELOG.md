...
- **Backtests run on the live series without blocking it, and fail loudly instead of mixing bar revisions**: `BarSeriesManager`, position sizing contexts, and the Elliott/Wyckoff facades now use the caller's series (with its begin index) instead of an index-resetting copy, so signals and fills read the same bars. `BacktestExecutor` and walk-forward executions capture the series window once, run every strategy over exactly that window without holding a series lock, report it as an immutable `barSeries()`, and throw `IllegalStateException` if bars inside it are replaced, updated or evicted before they finish; appended bars are ignored. A live `ConcurrentBarSeries` therefore keeps accepting writes and serving readers during a backtest, and parallel scheduling applies to every series type.
- **`BarSeries.withReadLock` gives analysis code one coherent view through indicator views**: the existing `ConcurrentBarSeries.withReadLock` is now declared on `BarSeries` (running the action directly for other series), and indicator bar-series views delegate it. It is meant for short, bar-only reads: its Javadoc now warns that evaluating indicators inside it can deadlock with other readers once a writer waits.

### Fixed

- **Analyses price exits beyond the logical window end without moving benchmark bounds**: `CashFlow`, `CumulativePnL`, `Returns`, and `InvestedInterval` derive a private raw-addressable analysis end from actual trailing position activity, valuing those exits at their real fill prices. `TradingRecord.getEndIndex(BarSeries)` remains a logical execution bound for benchmarks and current-price criteria; explicit analysis bounds remain authoritative.
- **Curve consumers retain captured trailing exits**: `CashFlow` and `Returns` expose their captured analysis bounds (`getBeginIndex`/`getEndIndex`) for cross-package criteria; the end stops at the record's logical end (or a trailing exit beyond it), so Calmar and Omega no longer score the flat tail after an explicitly bounded record. Drawdown, Calmar, RoMaD, and Omega include materialized raw-bar exits beyond the logical window.
- **Retained-window analyses seed their first slot from pre-window entries**: when an entry predates the series' retained window, `CumulativePnL` seeds the first retained level with the full entry-to-first-retained-close move (guarded so an exit landing exactly on the first retained index is not double counted) and `Returns` anchors its return chain at the first retained close while computing loop starts in long arithmetic, so sliding-window series no longer produce NaN seeds or overflow-wrapped buffer writes; `OffsetNumBuffer` rejects inverted and fully disjoint range updates instead of wrapping terminal indexes.
- **Rolling-window and terminal-index hardening for borrowed-series analyses**: `Returns` and `InvestedInterval` capture the begin index their buffers were materialized against, so later rolling advances of the live series no longer rebase lookups or inherit old values onto never-calculated bars; `CashFlow`'s mark-to-market loop and `Drawdown`'s peak/trough scan iterate in long arithmetic so a same-bar position or curve ending at `Integer.MAX_VALUE` terminates instead of wrapping into billions of iterations; `Returns.getSize()` now reports the materialized return count including any trailing exit beyond the logical window end, feeding those returns to VaR/Expected Shortfall tail distributions; and the overflow-safe `addressableEndIndex` helper moved from the `PerformanceIndicator` interface (which would have published it as public API) to a package-private static on `OffsetNumBuffer`.
- Same-bar positions entering and exiting on the first retained bar now keep their realized return (the leading-slot placeholder overwrite no longer erases it). Seeded first-slot detection remains internal to risk criteria, avoiding a public bookkeeping API.
- Explicit record end bounds without trailing trades no longer extend analysis windows past the logical series end. Materialized invested flags remain addressable at their original indices even after those source bars are pruned.

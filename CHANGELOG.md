...
- **SpotBugs remediation**: Cleared targeted baseline entries for realtime bar equality, serializable bar builder factories, concurrent trade-builder ownership, walk-forward calibration map iteration, charting/cache path guards, forced garbage collection in backtest batching and benchmark harness flows, `DoubleNumFactory` singleton serialization, descriptor-only chain links, `DoubleNum`/`NaN` comparisons, performance-report ordering, lower-camel criteria factories, constructor-safety validation paths, backtest execution, support/resistance, ATR true-range ownership, average, MACD/PPO, edge/momentum helper, scoring/compression, oscillator, VWAP-derived, Elliott source-indicator and facade ownership, and Elliott scenario indicators, triggered/Elliott/time-rule constructor validation, Elliott suite analysis-result ownership, `BaseBar` time resolution nullness, obsolete Elliott macro demo private bridges, Renko, Alligator, fractal, swing-marker, and Marubozu constructor validation, base bar-series/cache constructor validation, and trade/trading-record constructor validation.

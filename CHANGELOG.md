...
### Added
- **Forecast models can share state without coupling to one estimator**: Added the `ForecastState` contract, reusable `ForecastFeatureExtractors` for common return-state feature vectors, and `Forecast.ofSummary(...)` for calibrated or model-produced summaries that do not manufacture sample paths.

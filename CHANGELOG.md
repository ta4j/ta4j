...
- **Security: descriptor type resolution no longer initializes arbitrary classes**: `Rule`/`Strategy` JSON and descriptor deserialization (`RuleSerialization`, `StrategySerialization`) now load the untrusted `type` with `Class.forName(name, false, loader)` and accept it only when it implements `Rule`/`Strategy`, so static initializers of unrelated classpath classes no longer run before the type check. A missing class and a class of the wrong kind now fail with the same `Unknown rule type`/`Unknown strategy type` message (no `ClassNotFoundException` cause), removing a classpath-enumeration oracle; `Descriptor type does not implement Rule/Strategy` is gone. Enum constructor arguments decode from the declared parameter type; serialized `__enumType_*` metadata is never resolved.
- `ComponentDescriptor#getTypeClass` resolves simple names in `org.ta4j.core` (for example `BaseStrategy`) instead of probing the nonexistent `org.ta4j.core.strategy` package.
- `ProgressCompletion.logging()`/`loggingWithMemory()` caller detection walks live stack frames (`StackWalker`) instead of re-loading frame class names, so callers from other class loaders get their own logger and no class is loaded or initialized by name.

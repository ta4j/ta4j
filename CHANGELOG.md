...
- **Report-producing analysis demos can run on demand or on schedule**: The standalone `analysis-demo` JUnit tag now covers the live Elliott Wave BTC/USD macro report and the SPDR LPPL sector rotation report. The dedicated GitHub Actions workflow runs weekly by default, keeps daily/monthly schedules opt-in, uploads generated report/cache artifacts, and opens an `automation/lppl-spdr-reference-data` PR when scheduled or manual LPPL refreshes change committed SPDR reference data.

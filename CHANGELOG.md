## Unreleased
- **Futures review round 38**: retained futures entries preserve their aggregate basis only when every entry fill is retained; deferred or as-of-filtered fills derive a basis from the retained exposure, inverse entry capital preserves the aggregate trade basis, and position-level linear costs exclude fills after the requested index. Deferred fill indexes remain deferred when trades are reindexed, imported cash flows restore the record event horizon, partial average-cost projection slices preserve their aggregate basis, unavailable futures mark prices propagate NaN, and net returns normalize deferred entries by executed entry notional.

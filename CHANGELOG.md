...
- **OpenCL forecast decisions batch into one device pipeline (`CF-107c`)**: the OpenCL provider now enqueues every decision's sampling, moments, sort, and quantile kernels into a single in-order command queue with one contiguous history upload, deterministic two-phase moment reductions, and event-derived total/transfer/kernel/reduction timing buckets, replacing four blocking drains and a per-decision history copy per evaluate call.
- **GPU acceleration widens automatic selection (`CF-107`)**: CUDA joins `ta4j.acceleration=auto` on compute-capability 12.0 GPUs for workloads above ~262K Monte Carlo samples (measured up to 68.9x provider-level and 3.9x end-to-end on an RTX 5090), and Metal automatic selection accepts any Apple GPU instead of only the M5 Max.

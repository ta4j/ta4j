...
### Added
- **Causal swing detection for sharp, rounded, and multi-scale turns**: Added `SlopeChangeSwingDetector` for sustained rolling-regression reversals and tolerant `SwingDetectors.multiScale(...)` consensus, while ZigZag detection now uses intrabar highs/lows with pivot-anchored thresholds and fractal plateaus resolve to one deterministic midpoint.

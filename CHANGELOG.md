...
- **Unified parameter research pipeline (`CF-455`)**: Added `ParameterResearch`, a budget-exact hyperparameter search workflow: a reshaped `Builder` over typed integer, decimal, boolean, and categorical `ParameterDomain`s feeds three seeded engines — a lazy Cartesian grid, a hand-rolled genetic engine (tournament selection, domain-aware crossover and mutation, elitism), and a global-best particle swarm — through an `ObjectiveFunction` seam with per-candidate metric maps and explicit invalid/failure outcomes. A run-local cache keyed by candidate, dataset, window, and phase means duplicates, cache hits, normalized aliases, and re-proposed elite slots never consume the unique-evaluation budget; the final generation or swarm batch is truncated instead of overshooting it, and a proposal cap bounds normalization and validation work including raw re-proposals of an already-proposed raw id. An optional holdout window reserves its evaluations out of the budget and rebuilds the top-K training candidates with per-candidate train/holdout scores, deltas, and metric maps; deterministic ordering (primary score, then fewer repairs, then evaluation ordinal, then canonical id) governs leaderboards and engine internals; `TerminationReason` distinguishes budget, space, iteration, target, stagnation, cancel, and no-valid-candidate ends; and the `ParameterResearchReport` carries the search plan, seed, objective fingerprint, windows, counts, timings, warnings, and leaderboards. Normalizers can repair values onto declared positions, out-of-domain repairs keep their raw-string identity, snapshots fail fast when the backing series changes mid-run, and interruption between batches reports `CANCELED`. `SimpleMovingAverageRangeBacktest` demonstrates a backtest-style objective, `RelationshipObjectiveSearchExample` tunes lead/lag, DTW, event-MI, or synchronization-F1 objectives with a one-line grid/GA/PSO switch, and both examples validate out of sample.
- **Parameter research pipeline hardened (`CF-455`)**: `ParameterResearch.Builder` now binds the candidate factory into a staged `CandidateStage<T>` that carries the factory's type, so maximize/minimize/run are only reachable with a correctly typed objective and a divergent candidate/objective pair is a configuration-time error instead of a bridge `ClassCastException` inside the run, and a second `candidate(...)` binding on the same builder is rejected so a retained stage can never rebind a differently typed workflow; normalizer and validator callbacks are re-invoked for every proposal event and raw re-proposals count against the run-wide proposal cap, so genetic elite slots and swarm re-projections keep stateful callback semantics while their callback work stays bounded; decimal canonicalization now derives positions from the parsed value — so every Double-parseable spelling, hex-float or padded or exponent form included, canonicalizes onto the same declared id — and scans the exact declared-position window (floored bounds, no clamping past the last declared index) so normalizer-mapped values beyond the declared range keep their raw identity instead of colliding with the last declared value; particle-swarm outcomes are keyed by occurrence rather than raw id, so two particles projecting onto one grid point keep their own normalized or rejected outcomes instead of the last observation leaking into a sibling particle's personal best; particle-swarm moves stop once the sweep cursor has no unseen cell left; and a canceled run finalizes the observed batch only when that batch actually processed proposals.

- **Advanced lead/lag, DTW, and event-dependence analysis (`CF-454`)**: Added `LeadLagCorrelationIndicator`, a rolling indicator that scans an inclusive lag range: `getValue(index)` is the selected lag's signed correlation and `getProfile(index)` returns the full `Profile` (one `Point` per lag, undefined lags retained) with every best lag and one deterministic selected lag under the existing `LaggedCorrelationIndicator` sign convention; `DynamicTimeWarpingDistanceIndicator`, a bounded two-row-DP shape distance with z-score or raw normalization, absolute or squared local distance, an explicit Sakoe–Chiba band (unconstrained opt-in), exact path-length normalization, and a short `Config.shapeComparison(radius)` recommended path; and `EventMutualInformationEvaluator`, which measures how much a continuous predictor reduces uncertainty about a sparse Boolean event in an explicit current-or-future bar window, reporting raw and normalized MI in nats, target entropy, prevalence, and effective bin count (the reported prevalence must equal the factual positive-count ratio exactly). Event-MI target windows never cross the evaluation partition boundary, and equal-frequency binning never splits tied predictor values.
- **Windowed event synchronization and F1 evaluation (`CF-453`)**: Added `EventSynchronizationIndicator`, a rolling `Indicator<Num>` that matches two sparse Boolean `Indicator` event streams one-to-one within asymmetric lead/lag tolerance windows (maximum cardinality, then minimum total and worst absolute lag, then deterministic index-ordered ties) and reports the F1 score of each closed trailing window, with a `getResult(index)` diagnostics view (counts, precision, recall, match provenance with signed offsets, unmatched indexes, and lag summaries). Event indexes are cached per source, the value is `NaN` until the window is fully available (stable-bar boundary and series range), and both-streams-empty windows are undefined; the matching engine, event adapters, and reconstruction machinery are package-private and all public classes live under `org.ta4j.core.indicators.statistics.event`. Window boundaries are never crossed, so a one-shot evaluation of an explicit range keeps training and validation windows isolated, and the Net Momentum × ZigZag confirmation workflow ships as a runnable example with a benchmark fixture over 100,000 bars.
...
- **Lead/lag and event-dependence analysis hardened (`CF-454`)**: Pearson correlation rescales its window by the largest absolute value before averaging, so extreme-but-finite values can no longer overflow the mean and turn an exactly correlated window undefined; and it centers on the window's first rescaled value, so near-endpoint windows (for example `1` and `Math.nextDown(1)`, whose exact mean is not representable and would round to the endpoint) keep their deviations instead of reporting ~0.7071 for a perfect correlation — z-score DTW normalization applies the same anchored, small-delta centering so near-endpoint windows stay shape invariant; equal-width event-MI bins are positioned from the full-span ratio `delta / span * binCount` so a subnormal width (rounded or underflowed to zero) can no longer shift samples between bins, and spans that overflow to infinity for finite extremes (for example `-Double.MAX_VALUE` and `Double.MAX_VALUE`) are binned in overflow-safe scaled coordinates instead of reporting the evaluation undefined, so opposite-end samples still separate into their endpoint bins and yield the full `ln(2)` mutual information for opposite labels; `Profile` rejects points whose sample count is neither 0 nor the profile's bar count; event-mutual-information evaluation returns an undefined result for empty series and for target-window spans at the JVM array ceiling instead of throwing; and the result invariant now pairs a constant target with NaN normalized mutual information and a non-constant target with a defined value; and DTW path selection now treats undefined (NaN) predecessor cells as unreachable, so a squared-cost underflow cell can no longer win the shorter-length tie-break and block a finite detour (a radius-1 band over `[0, Double.MIN_VALUE, 1]` versus `[0, 0, 1]` now reports the detour cost instead of NaN in double precision).

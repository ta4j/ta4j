...
- **Analyses price exits that land beyond the logical window end**: `TradingRecord.getEndIndex(BarSeries)` now returns a record's trailing end index when it is still addressable in raw storage (deriving the bound from position activity for records built directly from trades), and `CashFlow`, `CumulativePnL`, `Returns`, and `InvestedInterval` extend their value windows to that bound, so an exit filled on a bar hidden past the constrained window end is valued at its real fill price instead of being clamped back into the window.

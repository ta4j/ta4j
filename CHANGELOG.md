...
- **Static trendline option**: Support/resistance trendlines now expose a `dynamicRecalculation` flag so you can freeze the first established line and project it forward indefinitely, or keep the default dynamic recomputation on each new bar.
...
- **Support/resistance trendlines configuration**: Scoring no longer includes a "contains current price" bonus category; weights now sum to 1.0 across touch/extreme/outside/proximity/recency. Tolerance is configurable via percentage, absolute, or tick-size settings. The active segment (anchors, slope/intercept, swing-touch/outside counts, extreme-anchor flag, score) can be retrieved via `getCurrentSegment()`, and tolerance settings are exposed for UI/serialization consumers. Default caps now keep searches to the most recent 64 swing points and 2,048 candidate pairs (tuned against the bundled OHLC data); override the caps via the extended constructors when you need a wider search window.
- **Swing plateau invalidation**: Recent swing caches now purge stale swing highs/lows when flat tops/bottoms overrun the allowed equal-bar tolerance, preventing outdated pivots from feeding support/resistance trendlines.

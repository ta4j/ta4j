...
- `BaseBar` mutation ownership is now lazy for unretained bars and compact for a single retaining series, promoting to a weak map only for shared bars. Failed partial price updates invalidate every retaining series, not only the originating one.

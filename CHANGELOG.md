...
- **Execution helpers share live series while completed results freeze ownership**: `BarSeriesManager`, strategy execution, position sizing contexts, and the Elliott/Wyckoff facades observe the caller's live series directly so signal/fill decisions use one coherent revision; completed `BacktestExecutionResult` instances own immutable bar data for post-run criteria, preventing later rolling mutations from changing recorded scores.
- **Coherent read scopes survive read-only indicator views**: the existing `ConcurrentBarSeries.withReadLock` API is now available through `BarSeries`, allowing execution and analysis code to preserve the source lease without exposing a view's mutable delegate. Views also preserve `BarSeries.isConcurrent()`, keeping locked batches on the lease-owning thread while ordinary series retain parallel scheduling. Non-concurrent series execute read actions directly; concurrent scopes support same-thread nesting.

## Unreleased
- **Futures review round 80**: futures exposure ordering is a total order when timestamps are absent, imported positions adopt FIFO lots by entry chronology, recorded fee totals retain small components through nested aggregation, and proportional fee scaling rejects non-finite representable results.
...
- **Futures review round 68**: direct closed futures positions reject reversed executed entry and exit chronology by index or timestamp while deferred fills remain eligible for as-of analysis.
- **Futures review round 58**: spot as-of profit, cost basis, and invested intervals exclude deferred fills while preserving trade-level fees, modeled futures rates, quantity increments, and contract minimum bounds remain representable in their active numeric factories, FIFO futures cost basis preserves residual entry fees, fee totals remain representable in the fill factory, normalized affordability budgets are revalidated after conversion, proportional futures cash-flow allocations avoid intermediate overflow, imported deferred futures fills restore chronological event horizons, processed funding retries cannot alias another cash-flow type or later equal-time schedule entry, and missing futures execution times are rejected.
- **Futures review round 59**: maximum bounds use the contract factory; partial spot profit includes executed exit fees; mixed-factory fill aggregates reject lossy conversions; flat deferred futures positions skip mark validation.
- **Futures review round 60**: residual futures exposure uses scalar fallback executions, FIFO cost basis orders fills chronologically, and underflowed contract-size products are rejected.
- **Futures review round 61**: completed spot profit uses aggregate valuation only when both entry and exit fills are executed through the as-of index, so deferred entry fills cannot distort closed-position results.
- **Futures review round 62**: futures notional overflow, fallback-capital underflow, initial-return seeding, and excess futures exit exposure are rejected or accounted for at their source; inverse gross returns retain cancellation-safe arithmetic.
- **Futures review round 64**: inverse gross returns leave deferred exits unrealized, Calmar accepts explicit mark-price indicators, and invested-interval boundaries include the final executed exit bar.
- **Futures review round 63**: no-argument spot profit excludes deferred fills, fixed position sizes and cash-flow settlements reject lossy factory conversions, and supplied trading records provide their configured cost models to position sizing.
...
- **Futures review round 57**: cross-factory futures values are revalidated after conversion before arithmetic, and retained-bar snapshots preserve their original index range for windowed backtests.
- **Futures review round 56**: unbounded custom futures affordability searches now fail explicitly instead of assuming monotonic costs and returning an incomplete result.
...
- **Futures review round 46**: empty spot positions have zero as-of profit; affordability normalizes foreign budgets and checks configured maxima before fee-spike bracketing; deferred scalar futures entries carry no stop-loss risk; projected position splits omit unallocated cash flows; mark-to-market valuation does not use a retained bar after the requested cutoff.
...
- **Futures review round 38**: retained futures entries preserve their aggregate basis only when every entry fill is retained; deferred or as-of-filtered fills derive a basis from the retained exposure, inverse entry capital preserves the aggregate trade basis, and position-level linear costs exclude fills after the requested index. Deferred fill indexes remain deferred when trades are reindexed, imported cash flows restore the record event horizon, partial average-cost projection slices preserve their aggregate basis, unavailable futures mark prices propagate NaN, net returns normalize deferred entries by executed entry notional, and spot fills reject infinite prices and amounts while preserving NaN placeholders. Spot zero-price aggregation stays on the linear path, spot fees reject infinite values, processed funding comparisons normalize numeric factories, and futures payoff rejects exits beyond executed entry exposure.
- **Futures review round 39**: imported partial exits preserve each executed exit fill's index and price, and equal-time scheduled funding is validated as preceding the execution.
- **Futures review round 36**: a closed futures slice realizes the merged average-cost basis of its entry instead of the historical price of one fill; a recorder-assigned fill index saturates at `Integer.MAX_VALUE` instead of wrapping below an earlier terminal-index fill; funding and cash-flow allocation ignore deferred entry fills when matching closed exposure, so an executed exit is no longer consumed against unexecuted quantity; complete-close orders round every non-residual chunk down to the quantity increment instead of emitting off-grid fills; and `EXIT_IN_WINDOW` requires an executed exit fill inside the window instead of accepting an exit whose fills only span it. Windowed projections and imported positions preserve the entry trade's basis price when they are rebuilt in the record's number factory.
...
- **Futures-aware analysis and criteria**: `CashFlow`, `CumulativePnL`, and `Returns` accept an explicit capital valuation, `EquityCurveMode`, and `OpenPositionHandling`, and route futures records through position-level realized and unrealized economics instead of matched-trade products; `NetReturnCriterion`, `GrossReturnCriterion`, `TotalFeesCriterion`, `OpenPositionCostBasisCriterion`, `OpenPositionUnrealizedProfitCriterion`, and `ProcessCapabilityCriterion` apply contract-aware futures semantics, and `AnalysisCriterion.projectTradingRecord` projects a selected set of already-matched positions into a read-only record for windowed reporting.
...
- **Futures review round 11**: the `PositionSizer` affordability bisection is no longer capped at 80 iterations, so the largest affordable amount converges exactly to the number factory precision for very large budgets on both the continuous path (spot and increment-less futures) and the increment-grid path.
- **Futures review round 12**: the `PositionSizer` continuous affordability bisection gains a convergence guard that fails with `IllegalStateException` when a `Num` implementation with unbounded precision, e.g. an unbounded `MathContext` `DecimalNumFactory`, keeps the midpoint strictly between the bounds; finite-precision implementations converge far below the guard and the documented precision precondition now sits on `Context.maxAffordableAmount`. `PositionSizer` factory methods (`fixed`, `balance`, `kelly`) preserve caller amounts and principals exactly instead of normalizing them through `double`, so values beyond `2^53` survive the record initial-capital comparison unchanged for decimal number factories.
- **Futures review round 13**: `PositionSizer` factory validation no longer narrows `BigDecimal`/`BigInteger` inputs through `doubleValue()`, and the affordability budget finiteness check uses the domain `Num.isFinite`, so exact decimal values beyond the double range are preserved and accepted; `snapshotNumber` whitelists the JDK immutable number types and snapshots every other `Number` value at creation time, so caller-owned mutable inputs can no longer change after sizer construction; the affordability search convergence guard now covers about 9800 significant digits, so a 5000-digit `DecimalNumFactory` converges instead of failing with a spurious `IllegalStateException`.

- **Futures review round 14**: removed the pathological unbounded-precision affordability test from the default unit lane after the bounded 5000-digit convergence test established the supported finite-precision contract; factory Kelly validation now compares decimal inputs without double narrowing; the default-branch required status rule now points at the canonical Build and Verify gate after the retired standalone formatting workflow was consolidated; imported positions retain compatible holding-cost models and trade-index cursors, futures return accounting ignores pending fills and includes recorded extension executions, and extension-bar backtests continue until closeout; partial futures projections preserve residual open exposure, mixed number factories are normalized during fill aggregation, average-cost views retain original fill prices, modeled fees honor settlement conversions, extension-bar fee totals include executed fills, and complete stop-limit closes bypass volume caps.; return and risk views avoid discarded-history allocations and support nullable mark prices, futures numeric hashes follow numeric equality, imported event horizons are restored, and test scenarios now live with their owning APIs.
- **Futures review round 15**: scheduled funding is computed and allocated per held slice, partial average-cost closes retain entry-specific cash flows, single-position futures imports preserve cash flows, cross-factory fill economics and recorded fees remain consistent, all futures numeric value equality and hashes are factory independent, and complete off-grid futures remainder closes are not lost.
- **Futures review round 16**: futures analysis windows compare retained entry and exit quantities before treating a position as fully executed, closed-record fees remain unbounded while open-record fees retain extension coverage, and unused cost-basis parameters are removed.
- **Futures review round 24**: the as-of profit of a spot position whose exit is beyond the requested index now values the outstanding exposure at the requested mark price instead of leaking the later exit payoff; a dated futures contract stops filling a pending entry remainder at or after its trading cutoff while executions that reduce the open exposure stay allowed; single-position futures curve capital is derived from executed entry fills only, so deferred fills no longer dilute `Returns`, `CashFlow` and `CumulativePnL`; and a deserialized or list-imported record restores its recorded numeric factory from its retained numerics (rehydrated positions, initial capital, fees, margin rate) and normalizes imported positions into it, so merging lots recorded with different supported `Num` implementations no longer throws `ClassCastException`.

- **Futures review round 25**: `PositionSizer.fixed(Num)` re-wraps the requested amount into the record's number factory, so a value created by another factory no longer leaks foreign arithmetic (for example `DecimalNum` into `DoubleNum`) into a spot or futures record; the holding cost of a multi-fill futures entry accrues per executed entry fill over that fill's own exposure interval on both `Position.getHoldingCost()` and `Position.getHoldingCost(int)`, so a later fill is no longer charged from the first fill's index; windowed futures projections exclude deferred fills (`index < 0`) from the retained entry and exit quantities, so a window no longer reports P&L for exposure that never executed; and `OpenPositionCostBasisCriterion` derives the open futures cost basis from executed entry fills only.
- **Futures review round 26**: a multi-fill `operate(Trade)` exit batch under `SPECIFIC_ID` matching is validated against a simulated lot state before any fill is applied, so a fill that cannot be matched no longer leaves the earlier fills of the same batch published; the holding cost of a futures position is charged over each paired entry/closing fill exposure interval, so a close spread over several exit fills no longer charges the whole exposure to the first exit index; imported cash flow slices follow the live eligibility rule, so a fill executed exactly at a cash flow timestamp no longer receives a share of it and a missing execution timestamp is rejected; `FuturesOrderQuantitySupport.largestTradable` steps one increment below an increment-tolerance rounded cap that fails the maxima validation instead of returning an empty order; and the first reported return of a retained futures series is measured from the account capital, so profit realized before the retained head is no longer excluded from the reported growth.
- **Futures review round 27**: a windowed futures projection allocates each cash flow to the entry fill that owned it instead of splitting all flows pro rata over the closed/open quantity, so funding that applied to the retained fill is no longer moved to the still-open slice and dropped under `OpenPositionHandling.IGNORE`; the shared allocation kernel moves to `FuturesPositionAccounting.allocateCashFlowsByFill` and is reused by the live position lots; directly recorded funding and cash-flow events validate their bar index against the time order of every scheduled and applied event, so an event can no longer carry an index that precedes the event before it in time; `Returns.hasSeededFirstBarReturn()` marks the cumulative equity seed of a retained futures series and the risk criteria omit it, so a loss realized before the retained head no longer shows up as an in-window tail return in `ValueAtRiskCriterion` and `ExpectedShortfallCriterion`; a spot trade reports the instrument label its fills agree on and `TradeFill.forTrade` preserves it, so a trade recorded from labelled fills no longer loses its symbol and `BaseTrade.toString()` exposes it.

- **Futures review round 28**: a single-fill `SPECIFIC_ID` futures exit batch is validated against the simulated lot state before any fill is applied, so a rejected exit no longer advances the funding and event horizon of the record; a partial average-cost close reprices the merged lot at its basis, so retained fills no longer keep an intermediate entry price; the `PositionSizer` affordability bracket accounts for the marginal per-contract fee, so entry sizing honors per-contract rebates while a charge that does not grow with the contract count still leaves the bracket wide enough for the exact search; `CumulativePnL` and `CashFlow` settle a closed futures position once instead of valuing every earlier position at every bar; `BaseTrade` equality and hash include the recorded fills, so trades with the same entry/exit prices but different fill compositions are distinct; and an unfilled futures entry remainder cancelled by an opposing signal reports its signal index and requested amount instead of the cancellation index and remaining amount.

- **Futures review round 29**: batch fill validation seeds its index horizon from the trades a record already holds, so a later fill batch can no longer reintroduce an index below the recorded horizon; the recorded-fee total of a list-imported record counts executed fills only, so a deferred fill no longer contributes an unexecuted fee; funding allocations skip deferred entry and exit fills, so unexecuted quantity is no longer charged funding; windowed futures inclusion judges a closed position by the fill indexes it actually executed, so a position whose earliest exit fill precedes the window but which also exits inside it is no longer dropped; a list-imported record keeps the modeled transaction cost model of its positions instead of falling back to recorded fees; and `Position` equality and hash codes include the cash flows allocated to the position.
- **Futures review round 30**: imported positions with only deferred fills no longer charge unexecuted fees or allocate cash flows to planned quantity; futures event indices are validated against both scheduled events and executed fills, scheduled event-ID conflicts fail before state mutation, average-cost lots retain historical fill prices while exposing their merged basis, mismatched exits are completed only against executed entry exposure, windowed mark-to-market includes positions whose later exit fills cross the window, settled-position memoization no longer assumes settled positions form a prefix, and legacy null position cash flows remain equality/hash compatible.
- **Futures review round 32**: mark-to-market projections no longer double-count straddling closed positions; deferred fill timestamps and quantities are excluded from event horizons and adopted lots; realized curves seed pre-window open activity, gross returns normalize by executed entry exposure, and imported positions normalize every numeric component into the record factory.
- **Futures review round 33**: straddling exits are projected once, partially exited single-fill positions retain unmatched holding cost, terminal planned indices saturate at Integer.MAX_VALUE, and recorded fees take precedence over zero fixed-rate calculations.
## 0.25.0 (2026-09-07)
...
- **Shared local and hosted quality gate**: `scripts/run-full-build-quiet.sh` and its PowerShell counterpart run repository script fixtures, actionlint validation, and the all-non-demo test scope used by pull-request CI. The local default repairs license headers and formatting before verification; CI invokes the same entrypoint with `--validate-only` to reject drift without mutating its checkout. On Windows, the PowerShell entrypoint runs Maven natively and uses WSL for the shell-fixture preflight when available, with Git Bash as its fallback. Redundant standalone formatting, license, and actionlint workflows are removed.
...
- **Contributor verify and example runs now line up with the repo’s real commands**: You can now run the Maven `verify` gate to mirror CI’s advisory SpotBugs and JaCoCo feedback for `ta4j-core` and `ta4j-examples`, then launch examples straight from repo root with `./mvnw -pl ta4j-examples exec:java` for `Quickstart` or `-Dexec.mainClass=...` for a specific demo (`CF-119`).

...
- **Candle pattern foundation: adaptive morphology and shared thresholds**: Added `CandleBodyIndicator` (body magnitude, `|close - open|`) and `CandleRangeIndicator` (`high - low`), switched all internal candle-pattern indicators to the magnitude-based body primitive, tightened `BaseBar` OHLC validation so the upper shadow, lower shadow, body, and range geometry invariants always hold, and deprecated the signed `RealBodyIndicator` in favor of `CandleBodyIndicator` plus `Bar#isBullish()`/`Bar#isBearish()`. A package-private `CandleThresholdSupport` computes causal prior-window baselines (`SMAIndicator` of the preceding `averagePeriod` bars shifted by `PreviousValueIndicator`) for body and range measurements and exposes long/short-body, doji, long/short-shadow, and near classifiers with one documented recommended profile, so named patterns no longer invent body or shadow threshold machinery independently; `CandleThresholdSupport.forSeries(series, averagePeriod)` interns one support per distinct pair through a weakly-keyed identity table, so every pattern indicator over the same series shares one cached set of body/range/shadow primitives instead of recomputing baseline averages per instance, and construction-time validation is unchanged. Doji, bullish/bearish marubozu, hammer, hanging man, inverted hammer, and shooting star now use these causal prior-window thresholds with documented strict and inclusive boundaries and no longer embed ADX trend gates: compose any trend context explicitly in a strategy. Migrate removed ratio/configuration constructors to the new `(series, averagePeriod)` forms, or `(series, averagePeriod, rangeFactor)` for doji.

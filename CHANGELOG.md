...
- **SpotBugs remediation**: Cleared targeted baseline entries for realtime bar equality, serializable bar builder factories, walk-forward calibration map iteration, charting/cache path guards, forced garbage collection in backtest batching and benchmark harness flows, `DoubleNumFactory` singleton serialization, descriptor-only chain links, `DoubleNum`/`NaN` comparisons, performance-report ordering, `BaseBar` time resolution nullness, and obsolete Elliott macro demo private bridges.

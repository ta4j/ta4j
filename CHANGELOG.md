...
### Added
- **Causal swing detection for sharp, rounded, and multi-scale turns**: Added `SlopeChangeSwingDetector` for sustained rolling-regression reversals and tolerant `SwingDetectors.multiScale(...)` consensus, while ZigZag detection now locates pivots from intrabar highs/lows, confirms reversals from closes with pivot-anchored thresholds, and resolves fractal plateaus to one deterministic midpoint.
...
### Changed
- **Hands-off ta4j delivery no longer pauses at its own PR boundary**: Direct `$prd-deliver` and `$qa-and-ship` invocations now authorize the workflow's first non-default branch push and PR creation after local gates pass, while default-branch pushes and merges remain human-controlled.
...
### Fixed
- **Day-of-week rule serialization is deterministic**: `DayOfWeekRule` now preserves caller-provided day order, so descriptor and JSON round trips remain stable across JVM collection layouts.

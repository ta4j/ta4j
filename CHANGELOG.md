...
- **Candle pattern foundation: shared adaptive thresholds**: Every migrated candle pattern now shares one consistent, causal, cached body/shadow sizing model; single-candle patterns no longer apply ADX trend gates, and multi-candle patterns no longer rely on hidden trend requirements. The deprecated legacy `DarkCloudIndicator`/`PiercingIndicator` still apply explicit trend gates.

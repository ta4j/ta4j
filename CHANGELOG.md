...
- **Subnormal SPC updates combine convex terms once**: `CusumIndicator`'s deviation-scale recursion and `EwmaVarianceIndicator`'s shared mean and variance recursion now recombine exact finite operands whenever a difference-form result lies on the active primitive subnormal grid, including float-backed `Num` implementations, preventing separate half-ulp product roundings from publishing the wrong control scale, mean, or variance while preserving the fast difference form for normal-magnitude results.

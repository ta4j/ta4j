...
Examples:
  ...
  scripts/run-full-build-quiet.sh --goals test -- -Dgroups=integration -Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-metal,requires-display,requires-headless
EOF
}
...
if [[ "$DEFAULT_GATE" == "true" ]]; then
    DEFAULT_MAVEN_ARGS=("-Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-metal,requires-display,requires-headless")

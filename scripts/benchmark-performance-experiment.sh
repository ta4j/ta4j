...
usage() {
  cat <<'USAGE'
Usage:
  scripts/benchmark-performance-experiment.sh [base-ref] [candidate-ref] [output-dir] [-- runner args...]
...
The script runs ta4jexamples.performance.PerformanceExperimentRunner in
temporary worktrees for both refs, then compares performance.json artifacts with
ta4jexamples.performance.PerformanceComparison.
...
(
  ...
  mvn -q -pl ta4j-examples exec:java \
    -Dexec.mainClass=ta4jexamples.performance.PerformanceComparison \
    -Dexec.args="$comparison_args"
)

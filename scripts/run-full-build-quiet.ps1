...
Examples:
  ...
  scripts/run-full-build-quiet.ps1 --goals test -- -Dgroups=integration -Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-display,requires-headless
"@
}
...
if ($defaultGate) {
    $mavenArgs = @("-Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-display,requires-headless") + $mavenArgs
}

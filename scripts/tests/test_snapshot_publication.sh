...
for node in root.iter():
    ...
    if artifact in {"ta4j-parent", "ta4j-core", "ta4j-examples", "ta4j-cli", "ta4j-acceleration"}:
...
expected = {
    ...
    "ta4j-acceleration": (os.environ["STUB_ACCELERATION_RESOLVED_VERSION"], ""),
}
...
for artifact in ta4j-parent ta4j-core ta4j-examples ta4j-cli ta4j-acceleration; do
  ...
  elif [[ "$artifact" == "ta4j-cli" ]]; then
    resolved_version="$STUB_CLI_RESOLVED_VERSION"
    cp "$STUB_CLI_SOURCE" "$directory/$artifact-$resolved_version.jar"
  else
    resolved_version="$STUB_ACCELERATION_RESOLVED_VERSION"
    cp "$STUB_ACCELERATION_SOURCE" "$directory/$artifact-$resolved_version.jar"
  fi
...
printf '%s\n' "$url" >> "$STUB_CURL_LOG"
artifact=""
for candidate in ta4j-parent ta4j-core ta4j-examples ta4j-cli ta4j-acceleration; do
...
write_consumption_metadata_fixture() {
  ...
  local cli_resolved="$6" acceleration_resolved="$7"
  mkdir -p "$directory"
...
EOF
  cat > "$directory/ta4j-acceleration.xml" <<EOF
<metadata>
  <version>${version}</version>
  <versioning>
    <snapshotVersions>
      <snapshotVersion><extension>jar</extension><value>${acceleration_resolved}</value></snapshotVersion>
    </snapshotVersions>
  </versioning>
</metadata>
EOF
...
prepare_consumption_fixture() {
  local root="$1"
  local version="$2"
  mkdir -p "$root/ta4j-core/target" "$root/ta4j-examples/target" "$root/ta4j-cli/target" "$root/ta4j-acceleration/target"
  ...
  printf 'published acceleration\n' > "$root/ta4j-acceleration/target/ta4j-acceleration-${version}.jar"
}
...
run_consumption_fixture() {
  ...
  local acceleration_resolved="${STUB_METADATA_ACCELERATION_RESOLVED_VERSION:-${version%-SNAPSHOT}-20260714.120000-5}"
  write_metadata_curl_stub "$curl_stub"
  write_consumption_metadata_fixture "$metadata_dir" "$version" "$parent_resolved" "$core_resolved" "$examples_resolved" "$cli_resolved" "$acceleration_resolved"
  ...
  STUB_ACCELERATION_RESOLVED_VERSION="$acceleration_resolved" \
  ...
  STUB_ACCELERATION_SOURCE="${STUB_ACCELERATION_SOURCE:-$publisher_root/ta4j-acceleration/target/ta4j-acceleration-${version}.jar}" \

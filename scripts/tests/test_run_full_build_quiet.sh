...
test_default_invocation_uses_local_repair_gate() {
  ...
  expect_file_contains_line "$TMP/maven-args.txt" "-Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-display,requires-headless" "default invocation should include hosted non-demo, non-hardware tests"

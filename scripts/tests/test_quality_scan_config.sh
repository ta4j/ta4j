...
test_docs_point_to_real_maven_commands() {
  ...
  expect_file_contains "$ROOT/README.md" "./mvnw -B clean license:check formatter:validate verify -Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-metal,requires-opencl,requires-display,requires-headless" "README should document the non-mutating Maven-only equivalent"

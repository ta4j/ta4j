...
test_ci_reuses_canonical_local_gates() {
  ...
  expect_file_contains "$QUIET_BUILD" "-Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-metal,requires-display,requires-headless" "local default should include hosted non-demo, non-hardware tests"
...
test_docs_point_to_real_maven_commands() {
  ...
  expect_file_contains "$ROOT/README.md" "./mvnw -B clean license:check formatter:validate verify -Dta4j.excludedTestTags=analysis-demo,benchmark,requires-cuda,requires-metal,requires-display,requires-headless" "README should document the non-mutating Maven-only equivalent"

...
if (-not $Scenario) {
    ...
    try {
        ...
        foreach ($case in @('wsl', 'fallback', 'no-program-files', 'maven-failure', 'timeout')) {
            $output = & $shell -NoProfile -ExecutionPolicy Bypass -File $PSCommandPath -Scenario $case -FixtureRoot $root 2>&1
            $code = $LASTEXITCODE
            $expected = switch ($case) { 'maven-failure' { 7 }; 'timeout' { 124 }; default { 0 } }
            ...
            if ($case -in @('wsl', 'fallback', 'no-program-files')) {
                $calls = @(Get-Content "$root/calls.txt")
                $windows = [Environment]::OSVersion.Platform -eq [PlatformID]::Win32NT
                $expectedCalls = if ($windows -and $case -eq 'wsl') { @('wsl') } elseif ($windows -and $case -eq 'fallback') { @('wsl', 'bash') } else { @('bash') }
...
if ($args[0] -ne '--cd' -or $args[1] -ne $env:FIXTURE_ROOT -or
    $args[2] -ne 'bash' -or $args[3] -ne 'scripts/run-full-build-quiet.sh' -or
    $args[4] -ne '--preflight-only' -or $args.Count -ne 5) { throw 'Invalid WSL arguments' }

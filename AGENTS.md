...
- Do not ignore build errors even if you think they were pre-existing. Suppressing/ignoring/assuming or otherwise skipping over the errors is forbidden. **All errors that surface must be investigated and root cause resolved.** Surface to the user any fixes that require complex refactoring or design changes. Never skip over a failing test without explicit approval from user.

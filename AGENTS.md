...
- Update `CHANGELOG.md` only when the feature is ready for PR review: either when the user explicitly indicates the work is PR-ready or when the user asks to create a PR. Do not add changelog entries incrementally during stepwise feature development. When updating, use existing style/format conventions, avoid duplicates, and consolidate Unreleased section items as needed.

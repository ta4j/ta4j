...
class AnalysisCriterionIndicatorTest {
    ...
    void testCreatePartialLiveTradingRecordPreservesMetadata() {
        ...
        record.recordFill(
                new BaseTrade(0, Instant.EPOCH, barSeries.getBar(0).getClosePrice(), barSeries.numFactory().one(),
                        barSeries.numFactory().numOf(0.1), ExecutionSide.BUY, "order-1", "corr-1"));
        record.recordFill(
                new BaseTrade(1, Instant.EPOCH, barSeries.getBar(1).getClosePrice(), barSeries.numFactory().one(),
                        barSeries.numFactory().numOf(0.1), ExecutionSide.SELL, "order-1", "corr-1"));
        record.recordFill(
                new BaseTrade(2, Instant.EPOCH, barSeries.getBar(2).getClosePrice(), barSeries.numFactory().one(),

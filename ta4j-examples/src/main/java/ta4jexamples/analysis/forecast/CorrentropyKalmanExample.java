 ...
 * measurement near the predicted state contributes at full weight, weights fall
 * off once the deviation reaches the measurement-noise scale, and the kernel
 * saturates to outright rejection (zero weight) only beyond about eleven
 * standard deviations, while ordinary noise stays largely untouched.
 *
 * <p>
 * Only the trailing {@value #WALK_BARS} weekly bars are walked: the filter
 * initializes at the first bar of that window that yields valid ATR-derived Q/R
 * (after the ATR warm-up), so its state starts at the modern price scale

...
class Wave2OriginRuleTest {

    private final Wave2OriginRule rule = new Wave2OriginRule();

    @Test
    void passesWhenBullishWave2HoldsOrigin() {
        final RuleEvidence evidence = rule.evaluate(candidate(WaveDirection.BULLISH, 100, 120, 110, 145, 125, 160));

        assertThat(evidence.state()).isEqualTo(EvidenceState.PASS);
        assertThat(evidence.observations()).contains("origin price=100.0", "wave 2 end price=110.0");
    }

    @Test
    void failsWhenBullishWave2TouchesOrigin() {
        final RuleEvidence evidence = rule.evaluate(candidate(WaveDirection.BULLISH, 100, 120, 100, 145, 125, 160));

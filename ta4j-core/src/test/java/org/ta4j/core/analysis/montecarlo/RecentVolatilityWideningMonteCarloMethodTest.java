...
public class RecentVolatilityWideningMonteCarloMethodTest {
    ...
    public void calmWindowPreservesLargeFiniteSamplesWithoutCenterOverflow() {
        ...
        assertEquals(2, samples.size());
        assertEquals(FACTORY.numOf(1e308), samples.get(0));
        assertEquals(FACTORY.numOf(1e308), samples.get(1));
    }
    ...
    @Test
    public void calmWindowRejectsSampleThatOverflowsContextFactory() {
        MonteCarloMethod inner = context -> List.of(DECIMAL.numOf(new java.math.BigDecimal("1E400")));
        MonteCarloMethod method = new RecentVolatilityWideningMonteCarloMethod(inner, 2, 4d);
    ...
    private static List<Num> decimalWindowExact(String... values) {
        ...
        for (String value : values) {
            result.add(DECIMAL.numOf(new java.math.BigDecimal(value)));
        }
        return result;
    }

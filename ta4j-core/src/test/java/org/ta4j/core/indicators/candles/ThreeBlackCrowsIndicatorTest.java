    ...
    public void unstableBarsCoverSmaBaselineWindow() {
        for (int barCount : new int[] { 2, 4, 8 }) {
            ...
            for (int i = 0; i < barCount - 1; ++i) {
                boundarySeries.barBuilder().openPrice(20 + i).closePrice(21 + i).highPrice(22 + i).lowPrice(18 + i)
                        .add();
            }
            // White candle at index barCount - 1
            boundarySeries.barBuilder().openPrice(20 + barCount).closePrice(22 + barCount).highPrice(23.5 + barCount)
                    .lowPrice(18 + barCount).add();
            // Three black crows with very short lower shadows (0.1 < 2 * 0.1)
            boundarySeries.barBuilder().openPrice(23 + barCount).closePrice(20 + barCount).highPrice(23 + barCount)
                    .lowPrice(19.9 + barCount).add();
            boundarySeries.barBuilder().openPrice(22.5 + barCount).closePrice(19 + barCount).highPrice(22.5 + barCount)
                    .lowPrice(18.9 + barCount).add();
            boundarySeries.barBuilder().openPrice(22 + barCount).closePrice(18 + barCount).highPrice(22 + barCount)
                    .lowPrice(17.9 + barCount).add();

...
/**
 * Shared OHLC fixtures used for published-reference indicator assertions.
 *
 * <p>
 * Data source: the first 80 rows of
 * {@code tests/indicators/_testdata/quotes/default.csv} from
 * https://github.com/DaveSkender/Stock.Indicators.
 */
final class PublishedReferenceSeries {

    private static final String[][] STOCK_INDICATORS_DEFAULT_80_OHLC = { { "212.61", "213.35", "211.52", "212.8" },
    ...
    private PublishedReferenceSeries() {
    }

    static BarSeries stockIndicatorsDefaultSeries(NumFactory numFactory) {
        BarSeries series = new MockBarSeriesBuilder().withNumFactory(numFactory).build();
        for (String[] row : STOCK_INDICATORS_DEFAULT_80_OHLC) {
            series.barBuilder().openPrice(row[0]).highPrice(row[1]).lowPrice(row[2]).closePrice(row[3]).add();
        }
        return series;
    }

    static BarSeries copyUntilIndex(BarSeries source, NumFactory numFactory, int endIndexInclusive) {

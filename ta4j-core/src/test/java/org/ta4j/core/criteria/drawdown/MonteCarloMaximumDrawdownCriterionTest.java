...
public class MonteCarloMaximumDrawdownCriterionTest extends AbstractCriterionTest {
    ...
    private LiveTradingRecord buildLiveRecordWithOpenLot(org.ta4j.core.BarSeries series, boolean includeOpenLot) {
        ...
        if (includeOpenLot) {
            record.recordFill(0, new SimulatedTrade(0, Instant.EPOCH, series.getBar(0).getClosePrice(),
                    numFactory.numOf(10), null, ExecutionSide.BUY, "order-open", "open"));
        }
        record.recordFill(1, new SimulatedTrade(1, Instant.EPOCH, series.getBar(1).getClosePrice(), numFactory.one(),
                null, ExecutionSide.BUY, "order-1", "c1"));
        record.recordFill(2, new SimulatedTrade(2, Instant.EPOCH, series.getBar(2).getClosePrice(), numFactory.one(),
                null, ExecutionSide.SELL, "order-1", "c1"));
        record.recordFill(3, new SimulatedTrade(3, Instant.EPOCH, series.getBar(3).getClosePrice(), numFactory.one(),
                null, ExecutionSide.BUY, "order-2", "c2"));
        record.recordFill(4, new SimulatedTrade(4, Instant.EPOCH, series.getBar(4).getClosePrice(), numFactory.one(),
                null, ExecutionSide.SELL, "order-2", "c2"));
        record.recordFill(5, new SimulatedTrade(5, Instant.EPOCH, series.getBar(5).getClosePrice(), numFactory.one(),
                null, ExecutionSide.BUY, "order-3", "c3"));
        record.recordFill(6, new SimulatedTrade(6, Instant.EPOCH, series.getBar(6).getClosePrice(), numFactory.one(),
                null, ExecutionSide.SELL, "order-3", "c3"));

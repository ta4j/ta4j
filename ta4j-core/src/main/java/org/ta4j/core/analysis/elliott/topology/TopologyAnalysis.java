 ...
 *                             {@link TopologyStatus#FORMING}
 */
public record TopologyAnalysis(TopologyStatus status, WaveDirection direction, List<TopologyCandidate> candidates,
    ...
    public static TopologyAnalysis insufficientHistory(final String explanation) {
        return new TopologyAnalysis(TopologyStatus.INSUFFICIENT_HISTORY, null, List.of(), explanation, -1, -1);
    }

    public static TopologyAnalysis noMatch(final String explanation) {
        return new TopologyAnalysis(TopologyStatus.NO_MATCH, null, List.of(), explanation, -1, -1);
    }

    public static TopologyAnalysis forming(final WaveDirection direction, final int startBarIndex,
            final int endBarIndex, final String explanation) {
        return new TopologyAnalysis(TopologyStatus.FORMING, direction, List.of(), explanation, startBarIndex,
                endBarIndex);
    }

    public static TopologyAnalysis invalidated(final String explanation) {

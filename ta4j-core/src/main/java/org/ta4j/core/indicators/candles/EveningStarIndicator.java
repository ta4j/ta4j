 ...
 *
 * <p>
 * Matches the documented three-candle evening star morphology at {@code index}
 * for the candles at {@code index - 2} (first), {@code index - 1} (star), and
 * {@code index} (third):
 *
 * <pre>
 * first is bullish with a long body:
 *     body(first) &gt; priorAverageBody(first)
 * star has a short body and its real body gaps strictly above the first body:
 *     body(star) &lt; 0.5 * priorAverageBody(star)
 *     bodyBottom(star) &gt; bodyTop(first)
 * third is bearish, has a long body, and its close penetrates the first body
 * inclusively:
 *     body(third) &gt; priorAverageBody(third)
 *     close(third) &lt;= bodyTop(first) - penetration * body(first)
 * </pre>
 *
 * <p>
 ...
 * @since 0.22.2
 */
public class EveningStarIndicator extends CandlePatternIndicator {
     ...
     * @since 0.24.2
     */
    public EveningStarIndicator(final BarSeries series, final int averagePeriod, final double penetration) {
    ...
    protected Boolean calculate(int index) {
        ...
        return firstBar.isBullish() && thresholds.isLongBody(index - 2) && thresholds.isShortBody(index - 1)
                && !(bodyBottom(starBar).isZero() && firstBodyTop.isZero())
                && bodyBottom(starBar).isGreaterThan(firstBodyTop) && thirdBar.isBearish()
                && Num.isFinite(thirdBar.getOpenPrice()) && Num.isFinite(thirdClose) && thresholds.isLongBody(index)
                && (thirdClose.isLessThanOrEqual(penetrationLevel)

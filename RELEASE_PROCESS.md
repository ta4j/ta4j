...
5. Confirm release PRs are visible to release owner:
   ...
   - a non-author reviewer or team is requested when configured through `RELEASE_REVIEWERS` or `RELEASE_REVIEW_TEAMS`.
...
- The workflow auto-labels the PR with `release`, assigns it to `TheCookieLab`, and requests review from eligible `RELEASE_REVIEWERS` / `RELEASE_REVIEW_TEAMS` targets. If no reviewer list is configured, `RELEASE_OWNER` remains the default reviewer candidate. The PR author is always filtered from user reviewer requests because GitHub rejects self-review requests.
- A self-authored release PR with no eligible fallback reviewer/team leaves a warning in the logs, summary, and audit artifact instead of failing PR creation. Normal branch protection still requires one approving review, so configure a non-author reviewer/team to merge without an admin or bypass path.

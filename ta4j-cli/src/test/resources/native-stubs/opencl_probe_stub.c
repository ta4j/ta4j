 ...
 *
 * Test-only JNI stub for JniOpenClNativeBridge. It serves the payload stored
 * in /tmp/ta4j-probe-payload.txt on every nativeProbe call so tests can drive
 * the real Java-side probe parsing with arbitrary hostile payloads. The
...
static jstring payload(JNIEnv* environment) {
    FILE* file = fopen("/tmp/ta4j-probe-payload.txt", "r");
    if (file == NULL) {
        return (*environment)->NewStringUTF(environment, "ERROR||||||||0|stub payload file missing");
    }
    char buffer[4096];
...
Java_org_ta4j_cli_acceleration_internal_providers_JniOpenClNativeBridge_nativeProbe(JNIEnv* environment, jclass klass,
                                                                                    jint abi_version) {
    (void)abi_version;
    (void)klass;
...
Java_org_ta4j_cli_acceleration_internal_providers_JniOpenClNativeBridge_nativeEvaluate(
        JNIEnv* environment, jclass klass, jint abi_version, jint from_inclusive, jint decision_count, jint horizon,

...
- Prefer the bounded commands: `strategy backtest`, `strategy walk-forward`, `strategy sweep`, `indicator test`, `rule test`, `forecast run`, `performance run`, and `performance compare`. Use `catalog` instead of hard-coding discoverable aliases.
...
- Use `forecast run` for return-state inspection, Monte Carlo or analog projection, optional conformal calibration, and empirical or lognormal price conversion.
- Prefer `--reproducible --error-format JSON` for automation, and treat `status=partial` as an explicit operator decision rather than a normal success.

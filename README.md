...
**💡 Also new**: The `ta4j-cli` module gives you a stable local command surface for `backtest`, `walk-forward`, `sweep`, `indicator-test`, `rule-test`, and performance experiments. Build it with `mvn -pl ta4j-cli -am package`, then run the fat jar from `ta4j-cli/target/`.

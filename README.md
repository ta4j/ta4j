...
- **Contributor quality path:** Use `scripts/run-full-build-quiet.sh` on macOS/Linux/Git Bash/WSL or `scripts/run-full-build-quiet.ps1` on Windows PowerShell with Git Bash available on `PATH`. This is the canonical PR gate used by hosted CI: it runs actionlint, repository script fixtures, non-mutating license and formatter validation, and all non-demo Maven tests with blocking SpotBugs plus advisory JaCoCo feedback.
- **Maven-only equivalent:** Use `./mvnw -B clean license:check formatter:validate verify -Dta4j.excludedTestTags=analysis-demo`, `mvnw.cmd` with the same arguments, or system Maven 3.9+ when repository preflight checks are not needed.
- **Formatting/header repair:** Run `./mvnw -B license:format formatter:format` separately when validation reports a problem, review the resulting diff, then rerun the canonical gate. The canonical gate never repairs committed-source defects before checking them.
...
- Run `scripts/run-full-build-quiet.sh` or `scripts/run-full-build-quiet.ps1` before opening or updating a pull request. Hosted CI calls the same entrypoint, including actionlint, repository script fixtures, non-mutating source validation, all non-demo tests, blocking SpotBugs, and advisory JaCoCo; the quiet script preserves the raw Maven log while summarizing warnings, errors, exceptions, and unexpected output.

...
- **Contributor quality path:** Use `scripts/run-full-build-quiet.sh` on macOS/Linux/Git Bash/WSL or `scripts/run-full-build-quiet.ps1` on Windows PowerShell. This is the canonical PR gate used by hosted CI: it runs actionlint, repository script fixtures, non-mutating license and formatter validation, and all non-demo Maven tests with blocking SpotBugs plus advisory JaCoCo feedback.

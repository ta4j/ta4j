...
- [ ] **A/B Task 2 - NetMomentum Production Hardening + Decay Verification:** Implement one production-quality hardening change in `NetMomentumIndicator` (constructor/input validation, unstable-boundary handling, or NaN safety) and add deterministic tests proving both `decay=1` legacy parity and `decay<1` decay-window behavior.  
Task-specific winner check: both runs must include a real production change plus tests that cover unstable boundary edges and deterministic decay expectations. Better variant is the one that achieves full correctness with fewer correction cycles and lower time-to-green.

- [ ] **A/B Task 3 - Serialization Compatibility Contract Hardening:** Implement a concrete compatibility improvement in `ComponentSerialization`/`StrategySerialization` around component-field routing precedence (`rules` vs `components`) while preserving indicator label suppression and legacy `children`/`baseIndicators` inputs; add focused regression tests for mixed/legacy payloads.  
Task-specific winner check: both runs must preserve backward compatibility and pass serialization round-trip suites with new precedence/legacy regression tests. Better variant is the one that resolves compatibility edge cases with fewer failed test iterations and fewer full-build reruns.

- [ ] **A/B Task 4 - Named Strategy Error Diagnostics + Reconstruction Robustness:** Improve named-strategy reconstruction failure diagnostics (clearer context in thrown errors) while preserving `<SimpleName>_<param...>` reconstruction semantics; add tests for malformed vararg inputs and label-token edge cases.  
Task-specific winner check: both runs must keep successful reconstruction behavior unchanged and improve failure-path clarity, verified by positive and negative tests. Better variant is the one with complete constructor-contract and error-message coverage at lower tool/token cost.

- [ ] **A/B Task 5 - VolumeIndicator Rolling-Window Optimization (Cross-Package):** Implement a concrete enhancement to `VolumeIndicator` window computation (addressing the partial-sums optimization TODO) with corresponding unit/regression tests and Javadoc updates describing behavior and computational impact.  
Task-specific winner check: both runs must deliver merged-worthy production value (optimized implementation + tests + docs/Javadoc) and finish green. Better variant is the one that completes this cross-file/cross-package change with lower navigation overhead (fewer reads/searches) and faster time-to-green.

...
- **Run one of these before opening or updating a PR:** `scripts/run-full-build-quiet.sh` on macOS/Linux/Git Bash/WSL, `scripts/run-full-build-quiet.ps1` on Windows PowerShell with Git Bash available on `PATH`, or `./mvnw -B clean license:format spotless:apply verify` for the direct Maven path.
...
4. **Implement + test.** Run either supported full build before pushing:
   ...
   ```
   On Windows PowerShell, use `scripts/run-full-build-quiet.ps1` with Git Bash available on `PATH`, or use `mvnw.cmd -B clean license:format spotless:apply verify` for the direct Maven path. Bash is required only for the repository shell-fixture preflight provided by the quiet scripts. Review and commit formatting and license-header repairs; CI invokes the Bash entrypoint with `--validate-only` and fails if they were omitted.
